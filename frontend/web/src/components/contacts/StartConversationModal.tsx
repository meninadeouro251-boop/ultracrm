...
export default function StartConversationModal({
  ...
  const isWhatsAppCloud = useMemo(() => {
    ...
    // WhatsApp Cloud providers (not baileys, Ultra, Ultra_go)
    return !provider || !['baileys', 'ultra', 'ultra_go'].includes(provider);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // WhatsApp free text (baileys, Ultra) without template requires message
