...
const ChatArea = ({
  ...
  // Verificar se é um canal WhatsApp free text (baileys, Ultra, Ultra_go)
  ...
  // Recarregar inboxes quando uma conversa Z-API/Ultra é selecionada e o inbox não tem provider_connection
  useEffect(() => {
    ...
    const shouldRefreshProviderConnection =
      isWhatsAppChannel &&
      channelProvider &&
      ['zapi', 'ultra', 'ultra_go'].includes(channelProvider.toLowerCase()) &&
      inbox &&
  ...
  const isWhatsAppFreeTextChannel =
    isWhatsAppChannel &&
    channelProvider &&
    ['baileys', 'ultra', 'ultra_go'].includes(channelProvider.toLowerCase());

  // Verificar status de conexão do Z-API e Ultra
  ...
  const isUltraChannel =
    isWhatsAppChannel &&
    ['ultra', 'ultra_go'].includes(channelProvider?.toLowerCase() || '');
  const isDisconnected =
    (isZapiChannel || isUltraChannel) &&
  ...
  const getBannerLinkText = () => {
    // Não mostrar texto do link quando Z-API/Ultra está desconectado ou é canal free text
    // Esses providers não têm restrições de janela de 24 horas, apenas precisam estar conectados
    if (isDisconnected || isZapiChannel || isUltraChannel) {
      return undefined;
    }
  ...
  // Não mostrar link quando Z-API/Ultra está desconectado (apenas aviso de desconexão)
  const getBannerLink = () => {
    // Se Z-API/Ultra está desconectado, não mostrar link de restrições de 24 horas
    // Esses providers não têm restrições de janela de 24 horas, apenas precisam estar conectados
    if (isDisconnected || isZapiChannel || isUltraChannel) {
      return undefined;
    }
