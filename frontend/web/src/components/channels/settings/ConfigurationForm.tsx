...
// Privacy Settings Component for Ultra
const UltraPrivacySettings: React.FC<{
  instanceName: string;
  provider: string;
  ...
  useEffect(() => {
    const loadPrivacySettings = async () => {
      try {
        const response = await UltraApiService.fetchPrivacySettings(instanceName, provider);
        // Handle Ultra Go API response format: { data: { data: { GroupAdd, LastSeen, ... } } }
  ...
  const handlePrivacyUpdate = async (setting: string, value: string) => {
    ...
    try {
      ...
      // Map frontend keys to Ultra Go API format (camelCase)
      ...
      // For Ultra Go, include callAdd (not used in Ultra)
      if (provider === 'ultra_go') {
        apiPayload.callAdd = updatedSettings.groupadd; // Default to same as groupadd
      }

      await UltraApiService.updatePrivacySettings(instanceName, apiPayload, provider);
  ...
  // Ultra (Baileys): WAPrivacyValue, WAPrivacyOnlineValue, WAPrivacyGroupAddValue, WAReadReceiptsValue
  // Ultra_go: subset without contact_blacklist, no 'none' for groupadd
  const opt = (values: string[]) =>
    values.map(v => ({
      value: v,
      label: t(
        `settings.configuration.whatsapp.instance.privacy.options.${v === 'contact_blacklist'
          ? 'contactBlacklist'
          : v === 'match_last_seen'
            ? 'matchLastSeen'
            : v
  ...
  const privacyFieldOptions: Record<string, { value: string; label: string }[]> =
    provider === 'ultra_go'
      ...
      : {
        // Ultra (Baileys): types WAReadReceiptsValue, WAPrivacyValue, WAPrivacyOnlineValue, WAPrivacyGroupAddValue
        readreceipts: opt(['all', 'none']),
...
// Baileys WhatsApp Configuration ( Não Oficial )
const BaileysWhatsAppConfig: React.FC<{
  inbox: any;
  onUpdate: (data: any) => void;
}> = ({ inbox }) => {
  const { t } = useLanguage('channels');
  ...
  useEffect(() => {
    const loadInstanceStatus = async () => {
      try {
        ...
        const response = await BaileysService.getInstanceStatus(identifier);
        if (response?.status) {
          setInstanceStatus(response.status);

          // Close modal and show success if connected
          if (response.status === 'connected' && showQrModal) {
            setShowQrModal(false);
            setQrCode(null);
            toast.success(t('settings.configuration.whatsapp.instance.success.connected'));
          } else if (response.status === 'connecting' && response.qrCode) {
            // Update QR code while connecting
  ...
  const handleGenerateQR = async () => {
    ...
    try {
      ...
      const response = await BaileysService.createInstance(identifier);
      if (response?.qrCode) {
        setQrCode(response.qrCode);
        setShowQrModal(true);
  ...
  const handleDisconnect = async () => {
    ...
    setIsLoading(true);
    try {
      await BaileysService.logout(identifier);
        ...
        <CardContent className="p-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                Conecte seu número WhatsApp diretamente via Baileys
              </p>
            </div>
            <Badge variant={isConnected ? 'default' : 'secondary'}>
              {isConnected ? 'Conectado' : instanceStatus === 'connecting' ? 'Conectando...' : 'Desconectado'}
            </Badge>
          </div>
            ...
            <div className="mt-4 p-3 bg-green-50 dark:bg-green-950/20 rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-green-700 dark:text-green-400">
                  Instance "{identifier}" está conectada
                </span>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-slate-600 dark:text-slate-400">
                Instância: <strong>{identifier}</strong>
              </p>
...
// Ultra WhatsApp Configuration
const UltraWhatsAppConfig: React.FC<{
  inbox: any;
  onUpdate: (data: any) => void;
}> = ({ inbox, onUpdate }) => {
  const { t } = useLanguage('channels');
  ...
  useEffect(() => {
    const loadInstanceSettings = async () => {
      try {
        ...
        const isUltraGo = provider === 'ultra_go';
        const identifier = getIdentifier();

        if (!identifier) return;

        const response = await UltraApiService.getSettings(identifier, provider);
        ...
        if (settings) {
          if (isUltraGo) {
            ...
            setInstanceSettings({
              ...
              groupsIgnore: settings.ignoreGroups ?? settings.ignore_groups ?? false, // Ultra Go uses ignoreGroups
              ...
              syncFullHistory: false, // Ultra Go doesn't have this
              readStatus: !ignoreStatus, // Invert ignoreStatus to readStatus
            });
          } else {
            // Ultra (normal) settings mapping
            setInstanceSettings({
    ...
    // Load profile settings for Ultra Go
    const loadProfileSettings = async () => {
      try {
        if (inbox.provider !== 'ultra_go') return;
        const identifier = getIdentifier();
        if (!identifier) return;

        const response = await UltraApiService.getProfile(identifier, inbox.provider);
  ...
  useEffect(() => {
    const loadInstanceStatus = async () => {
      try {
        ...
        const response = await UltraApiService.getInstances(identifier, provider);
  ...
  const handleGenerateQR = async () => {
    ...
    try {
      ...
      const response = await UltraApiService.getQRCode(identifier, provider);
      if (response?.base64) {
  ...
  const handleUpdateInstanceSettings = async () => {
    ...
    try {
      ...
      if (isUltraGo) {
        // Ultra Go: convert readStatus to ignoreStatus (invert) and groupsIgnore to ignoreGroups
        settingsPayload = {
        ...
        // Ultra (normal): use settings as-is
        settingsPayload = {
          rejectCall: instanceSettings.rejectCall,
          msgCall: instanceSettings.msgCall,
      ...
      // Update via Ultra API (support both Ultra and Ultra_go)
      await UltraApiService.updateSettings(identifier, settingsPayload as any, provider);
  ...
  const handleUpdateProfileName = async () => {
    ...
    try {
      ...
      await UltraApiService.updateProfileName(
        identifier,
  ...
  const handleUpdateProfileStatus = async () => {
    ...
    try {
      ...
      await UltraApiService.updateProfileStatus(
        identifier,
  ...
  const handleUpdateProfilePicture = async () => {
    ...
    try {
      ...
      await UltraApiService.updateProfilePicture(
        identifier,
  ...
  const handleRemoveProfilePicture = async () => {
    ...
    try {
      ...
      await UltraApiService.removeProfilePicture(identifier, provider);
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      await UltraApiService.logout(identifier, provider);
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                  ...
                  {/* Only show syncFullHistory for Ultra (not Ultra Go) */}
                  {inbox.provider !== 'ultra_go' && (
                    <div className="flex items-center justify-between">
                      <label className="text-sm font-medium">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      ...
                      onCheckedChange={checked => {
                        if (inbox.provider === 'ultra_go') {
                          // For Ultra Go, invert the value (ignoreStatus is opposite of readStatus)
...
const ConfigurationForm: React.FC<ConfigurationFormProps> = ({ inbox, onUpdate }) => {
  ...
  const isUltraChannel = inbox.provider === 'ultra';
  const isUltraGoChannel = inbox.provider === 'ultra_go';
  const isZapiChannel = inbox.provider === 'zapi';
  const isBaileysChannel = inbox.provider === 'baileys';
  ...
  // WhatsApp Ultra
  if (isWhatsAppChannel && isUltraChannel) {
    return <UltraWhatsAppConfig inbox={inbox} onUpdate={handleUpdate} />;
  }

  // WhatsApp Ultra Go
  if (isWhatsAppChannel && isUltraGoChannel) {
    return <UltraWhatsAppConfig inbox={inbox} onUpdate={handleUpdate} />;
  }
  ...
  if (isWhatsAppChannel && isBaileysChannel) {
    return <BaileysWhatsAppConfig inbox={inbox} onUpdate={handleUpdate} />
  }
