...
interface UltraFormProps {
  form: FormData;
  onFormChange: (key: string, value: string | boolean) => void;
  hasUltraConfig: boolean;
}

export const UltraForm = ({ form, onFormChange, hasUltraConfig }: UltraFormProps) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {!hasUltraConfig && (
        <>
          <FormField
            label={t('UltraForm.fields.baseUrl.label')}
            value={getStr('api_url')}
            onChange={value => onFormChange('api_url', value)}
            placeholder={t('UltraForm.fields.baseUrl.placeholder')}
            type="url"
            required
          />

          <FormField
            label={t('UltraForm.fields.apiKey.label')}
            value={getStr('admin_token')}
            onChange={value => onFormChange('admin_token', value)}
            placeholder={t('UltraForm.fields.apiKey.placeholder')}
            type="password"
            required
          />
        </>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4" data-tour="whatsapp-credentials">
        <FormField
          label={t('UltraForm.fields.displayName.label')}
          value={getStr('display_name')}
          onChange={handleDisplayNameChange}
          placeholder={t('UltraForm.fields.displayName.placeholder')}
          helpText={t('UltraForm.fields.displayName.helpText')}
          required
        />
        <FormField
          label={t('UltraForm.fields.channelName.label')}
          value={getStr('name')}
          onChange={value => onFormChange('name', value)}
          placeholder={t('UltraForm.fields.channelName.placeholder')}
          helpText={t('UltraForm.fields.channelName.helpText')}
          required
          readOnly
        />
      </div>
      ...
      {/* Proxy Configuration */}
      <FormSection
        title={t('UltraForm.sections.proxy.title')}
        className="bg-gray-50/10 border-gray-200/20"
      >
        <div className="mb-4">
          <FormCheckbox
            label={t('UltraForm.sections.proxy.enableProxy')}
        ...
        {form.proxy_enabled && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField
              label={t('UltraForm.sections.proxy.host.label')}
              value={getStr('proxy_host')}
              onChange={value => onFormChange('proxy_host', value)}
              placeholder={t('UltraForm.sections.proxy.host.placeholder')}
            />
            <FormField
              label={t('UltraForm.sections.proxy.port.label')}
              value={getStr('proxy_port')}
              onChange={value => onFormChange('proxy_port', value)}
              placeholder={t('UltraForm.sections.proxy.port.placeholder')}
            />
            <div>
              <label className="text-sm font-medium text-sidebar-foreground/80">
                {t('UltraForm.sections.proxy.protocol')}
              </label>
              <select
            ...
            <FormField
              label={t('UltraForm.sections.proxy.username.label')}
              value={getStr('proxy_username')}
              onChange={value => onFormChange('proxy_username', value)}
              placeholder={t('UltraForm.sections.proxy.username.placeholder')}
            />
            <div className="md:col-span-2">
              <FormField
                label={t('UltraForm.sections.proxy.password.label')}
                value={getStr('proxy_password')}
                onChange={value => onFormChange('proxy_password', value)}
                placeholder={t('UltraForm.sections.proxy.password.placeholder')}
                type="password"
              />
            </div>
          </div>
      ...
      >
        ...
        <div className="mt-4">
          <FormField
            label={t('UltraForm.sections.instance.msgCall.label')}
            value={getStr('msgCall')}
            onChange={value => onFormChange('msgCall', value)}
            placeholder={t('UltraForm.sections.instance.msgCall.placeholder')}
          />
        </div>
      </FormSection>
    </div>
