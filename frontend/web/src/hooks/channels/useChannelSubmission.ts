...
import {
  ...
  WhatsappUltraGoPayload,
  WhatsappUltraPayload,
  WhatsappTwilioPayload,
  WhatsappNotificamePayload,
  WhatsappZapiPayload,
  Inbox,
} from '@/types/channels/inbox';
import ultraService from '@/services/channels/ultraService';
import ultraGoService from '@/services/channels/ultraGoService';
...
export const useChannelSubmission = (form?: FormData) => {
  ...
  const cleanupPendingInstance = useCallback(() => {
    ...
    ultraGoService.deleteInstance(pending).catch(() => { });
  }, []);
  ...
  const testConnection = async (
    ...
    form: FormData,
    config: { hasUltraConfig: boolean; hasUltraGoConfig: boolean },
  ) => {
    ...
    try {
      ...
      // Health check para Ultra API
      if (selectedProvider.id === 'ultra') {
        // 🔒 SECURITY: When using global config, don't send api_url/admin_token from frontend
        const useGlobalConfig = config.hasUltraConfig === true;
        ...
        try {
          ...
          if (!useGlobalConfig) {
            ...
            const healthOk = await ultraService.healthCheck(apiUrl);
            if (!healthOk) {
              result = {
                success: false,
                error:
                  'Health check falhou. Verifique se a URL da Ultra API está correta e acessível.',
              };
          ...
          await ultraService.verifyConnection(verifyPayload);
      ...
      } else if (selectedProvider.id === 'ultra_go') {
        // 🔒 SECURITY: When using global config, don't send api_url/admin_token from frontend
        const useGlobalConfig = config.hasUltraGoConfig === true;
        ...
        try {
          ...
          if (!useGlobalConfig) {
            ...
            // Run health check for custom URL
            const healthOk = await ultraGoService.healthCheck(apiUrl);
            if (!healthOk) {
              result = {
                success: false,
                error:
                  'Health check falhou. Verifique se a URL da Ultra Go está correta e acessível.',
              };
              setHealthCheckPassed(false);
              setIsTesting(false);
              toast.error(result.error);
          ...
          // Backend will run health check if using global config
          await ultraGoService.verifyConnection(verifyPayload);
          result = { success: true, message: 'Conexão verificada com sucesso' };
          setHealthCheckPassed(true);
        } catch (error) {
          result = { success: false, error: (error as Error).message };
    ...
    try {
      ...
      switch (selectedChannel.type) {
        ...
        case 'whatsapp': {
          ...
          if (selectedProvider.id === 'whatsapp_cloud') {
          ...
          } else if (selectedProvider.id === 'ultra') {
            // 🔒 SECURITY: When using global config, don't send api_url/admin_token from frontend
            const useGlobalConfig = config.hasUltraConfig === true;
            ...
            payload = {
              name: getStr(form, 'name') || 'WhatsApp Ultra',
              display_name:
                getStr(form, 'display_name') || getStr(form, 'name') || 'WhatsApp Ultra',
              channel: {
                type: 'whatsapp',
                phone_number: getStr(form, 'phone_number'),
                provider: 'ultra',
            ...
            const verify = await ultraGoService.verifyConnection(verifyPayload);
            ...
            payload = {
              name: getStr(form, 'name') || 'WhatsApp Ultra Go',
              display_name:
                getStr(form, 'display_name') || getStr(form, 'name') || 'WhatsApp Ultra Go',
              channel: {
                type: 'whatsapp',
                phone_number: getStr(form, 'phone_number'),
                provider: 'ultra_go',
      ...
      try {
        ...
        // createChannel failed — instance exists on Ultra Go but no inbox in CRM.
        if (pendingInstance) {
          ultraGoService.deleteInstance(pendingInstance).catch(() => { });
        }
        throw createError;
      }
