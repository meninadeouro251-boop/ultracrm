...
export const useChannelValidation = () => {
  ...
  const validateUltra = (form: FormData, hasUltraConfig: boolean) => {
    if (!getStr(form, 'name').trim()) {
      toast.error('Nome do canal é obrigatório');
      return false;
    }
  ...
  const validateUltraGo = (form: FormData, hasUltraGoConfig: boolean) => {
    if (!getStr(form, 'name').trim()) {
      toast.error('Nome do canal é obrigatório');
      return false;
    }
    ...
    switch (channelType) {
      ...
      case 'whatsapp':
        ...
        switch (providerId) {
          ...
          case 'ultra':
            return validateUltra(form, config?.hasUltraConfig ?? false);
          case 'ultra_go':
            return validateUltraGo(form, config?.hasUltraGoConfig ?? false);
          case 'zapi':
