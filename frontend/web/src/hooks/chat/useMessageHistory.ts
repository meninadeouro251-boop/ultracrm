...
export const useMessageHistory = ({ conversationId, enabled = true }: UseMessageHistoryProps) => {
  ...
  const loadInitialMessages = useCallback(async () => {
    ...
    try {
      // Ultra backend limits: 20 messages for latest, 20 for before, 100 for after
      ...
      // Ultra sempre retorna no máximo 20 mensagens iniciais
  ...
  const loadMoreMessages = useCallback(async () => {
    ...
    try {
      // Ultra backend: usar 'before' para carregar mensagens anteriores (limit 20)
      const response = await chatService.getMessages(conversationId, {
      ...
      if (olderMessages.length > 0) {
        setState(prev => {
          ...
          // Ultra retorna no máximo 20 mensagens com 'before'
          // Se retornou 20, provavelmente há mais mensagens
