...
export const useGlobalWebSocket = (handlers: GlobalWebSocketHandlers) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('Ultra:auth-lost', handleAuthLost);
    return () => {
      window.removeEventListener('Ultra:auth-lost', handleAuthLost);
    };
  }, [disconnect]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('Ultra:notification', handleNotificationEvent as EventListener);

    return () => {
      window.removeEventListener('Ultra:notification', handleNotificationEvent as EventListener);
    };
  }, []);
