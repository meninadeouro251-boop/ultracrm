...
class ActionCableService {
  ...
  //   // Notifications are handled through RoomChannel in Ultra
  //   // No separate NotificationChannel needed
  // }

  // private subscribeToPresence() {
  //   // Presence is handled through RoomChannel in Ultra
  ...
  private handleEventMessage(data: any) {
    ...
    switch (event) {
      ...
      case 'conversation.updated':
        window.dispatchEvent(new CustomEvent('Ultra:conversation', { detail: payload }));
        break;

      case 'message.created':
      case 'message.updated':
        window.dispatchEvent(new CustomEvent('Ultra:message', { detail: payload }));
        break;

      case 'conversation.typing_on':
      case 'conversation.typing_off':
        window.dispatchEvent(new CustomEvent('Ultra:typing', { detail: payload }));
        break;

      case 'conversation.status_changed':
        window.dispatchEvent(new CustomEvent('Ultra:status', { detail: payload }));
        break;

      case 'conversation.contact_changed':
        window.dispatchEvent(new CustomEvent('Ultra:contact', { detail: payload }));
        break;

      case 'presence.update':
        window.dispatchEvent(new CustomEvent('Ultra:presence', { detail: payload }));
        break;

      default:
        window.dispatchEvent(new CustomEvent('Ultra:event', { detail: { event, payload } }));
    }
  }

  // private handleNotification(data: any) {
  //   window.dispatchEvent(new CustomEvent('Ultra:notification', { detail: data }));
  ...
  //   window.dispatchEvent(new CustomEvent('Ultra:presence_update', { detail: data }));
  // }
