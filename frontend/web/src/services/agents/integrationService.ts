...
class IntegrationService {
  ...
  async getIntegration(agentId: string, provider: string): Promise<AgentIntegration> {
    const response = await ultraaiApi.get(`/agents/${agentId}/integrations/${provider}`);
    return extractData<AgentIntegration>(response);
  }
  ...
  async upsertIntegration(
    ...
    const response = await ultraaiApi.post(`/agents/${agentId}/integrations`, data);
    return extractData<AgentIntegration>(response);
  }
  ...
  async deleteIntegration(agentId: string, provider: string): Promise<void> {
    await ultraaiApi.delete(`/agents/${agentId}/integrations/${provider}`);
  }
}
