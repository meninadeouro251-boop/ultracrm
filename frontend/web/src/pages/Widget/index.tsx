...
export default function Widget() {
  ...
  const syncConversationId = (id: string | null) => {
    ...
    if (typeof window !== 'undefined') {
      sessionStorage.setItem('ultra_widget_conversation_id', id);
    }
  };
  ...
  const ensureConversationForSend = async (
    ...
    if (!forceNewConversationRef.current) {
      const storedConversationId = sessionStorage.getItem('ultra_widget_conversation_id');
  ...
  const handleEndConversation = async () => {
    ...
    if (typeof window !== 'undefined') {
      sessionStorage.removeItem('ultra_widget_conversation_id');
    }
  ...
  useEffect(() => {
    ...
    const listener = (e: MessageEvent) => {
      if (typeof e.data !== 'string' || e.data.indexOf('ultra-widget:') !== 0) return;
      try {
        const msg = JSON.parse(e.data.replace('ultra-widget:', '')) as {
          event: string;
          [key: string]: any;
        };
        switch (msg.event) {
          ...
          case 'set-custom-attributes':
            ...
            if (msg.custom) {
              sessionStorage.setItem('ultra_widget_custom_attrs', JSON.stringify(msg.custom));
            }
            break;
          default:
            break;
                    ...
                    return {
                      ...
                      attachments: m.attachments
                        ? m.attachments.map((att: any) => ({
                          ...
                          fallback_title:
                            att.fallback_title ||
                            `Arquivo (${att.file_size ? Math.round(att.file_size / 1024) : 0
                            } KB)`,
                        }))
                        : undefined,
                    };
              ...
              return {
                ...
                attachments: m.attachments
                  ? m.attachments.map((att: any) => ({
                    ...
                    fallback_title:
                      att.fallback_title ||
                      `${t('chat.file')} (${att.file_size ? Math.round(att.file_size / 1024) : 0
                      } KB)`,
                  }))
                  : undefined,
              };
  ...
  const handlePreChatSubmit = async (preChatData: PreChatSubmissionData) => {
    ...
    try {
      ...
      const storedUser = sessionStorage.getItem('ultra_widget_user');
      const storedAttrs = sessionStorage.getItem('ultra_widget_custom_attrs');
