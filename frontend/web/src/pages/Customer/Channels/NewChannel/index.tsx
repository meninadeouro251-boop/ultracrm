...
export default function NewChannel() {
  ...
  const handleProviderSelectWithValidation = (provider: ProviderType) => {
    if (selectedChannel?.type === 'whatsapp') {
      ...
      if (provider.id === 'ultra' && !hasUltraConfig) {
        return toast.error(t('newChannel.channelGrid.notConfiguredTooltip'));
      }
      if (provider.id === 'ultra_go' && !hasUltraGoConfig) {
        return toast.error(t('newChannel.channelGrid.notConfiguredTooltip'));
      }
    }
  ...
  const shouldShowTestConnection = (): boolean => {
    return !!(
      ...
      selectedProvider &&
      ['twilio', 'notificame', 'ultra', 'ultra_go'].includes(selectedProvider.id)
    );
  };
            ...
            <ProviderSelection
              ...
              isDisabled={providerId => {
                if (selectedChannel?.type === 'whatsapp') {
                  ...
                  if (providerId === 'ultra') return !hasUltraConfig;
                  if (providerId === 'ultra_go') return !hasUltraGoConfig;
                }
                if (selectedChannel?.type === 'email') {
                ...
                const gated =
                  (selectedChannel?.type === 'whatsapp' &&
                    ((providerId === 'whatsapp_cloud' && !canWpCloud) ||
                      (providerId === 'ultra' && !hasUltraConfig) ||
                      (providerId === 'ultra_go' && !hasUltraGoConfig))) ||
                  (selectedChannel?.type === 'email' &&
                        ...
                        isDisabled={
                          ...
                          (selectedProvider?.id === 'whatsapp_cloud' &&
                            (!form.name ||
                              ...
                              !form.waba_id)) ||
                          // Desabilita salvar se for Ultra ou Ultra Go e o health check não passou
                          ((selectedProvider?.id === 'ultra' ||
                            selectedProvider?.id === 'ultra_go') &&
