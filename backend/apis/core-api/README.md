# Ultra CRM Core Service

Microserviço de IA integrado ao ultralution que fornece funcionalidades avançadas de inteligência artificial, incluindo ferramentas personalizadas, agentes inteligentes e gerenciamento de chaves API.
...
cd ultra-ai-core-service-community
...
O serviço utiliza o mesmo banco de dados do ultralution para integração perfeita:
...
DB_USER=postgres
DB_PASSWORD=root
DB_NAME=ultra_community  # Mesmo banco do ultralution
DB_SSLMODE=disable
```

### 2. Integração ultralution

```bash
# URL base do ultralution (sem barra no final)
ultraLUTION_BASE_URL=http://localhost:3000

# JWT Secret deve coincidir com o SECRET_KEY_BASE do ultralution
JWT_SECRET_KEY=your_ultralution_secret_key_base_here
```
...
```
ultra-ai-core-service-community/
...
## 🔒 Autenticação ultraAuth

O serviço utiliza **Bearer Token** para autenticação via tokens do ultraAuth, garantindo integração perfeita com o sistema existente.

### Como Funciona

1. **Token Validation**: Valida tokens via endpoint `/api/v1/me` do ultraAuth
...
O middleware (`internal/middleware/ultra_auth.go`) implementa:

- ✅ **Validação automática** via API do ultraAuth
- ✅ **Extração de contexto** (user_id, email, name, user completo, accounts)
- ✅ **Bearer Token** único formato suportado
- ✅ **Logs detalhados** para debugging

### Estrutura de Resposta do ultraAuth
...
```env
# .env
ultraLUTION_BASE_URL=http://localhost:3000  # URL base do ultralution (sem barra no final)
JWT_SECRET_KEY=your_ultralution_secret_key_base  # Deve coincidir com ultralution
```
...
- **Banco Compartilhado**: Utiliza o mesmo PostgreSQL do ultralution
- **Prefixo de Tabelas**: Todas as tabelas prefixadas com `ultra_core_` para evitar conflitos
- **Chaves Estrangeiras**: Referencia `accounts(id)` do ultralution para multi-tenancy
- **Segurança de Migração**: Backup da tabela `schema_migrations` do Rails antes de executar migrações Go

### Tabelas Principais

- `ultra_core_agents` - Agentes IA com diferentes tipos
- `ultra_core_custom_tools` - Ferramentas personalizadas HTTP
- `ultra_core_api_keys` - Armazenamento criptografado de credenciais
- `ultra_core_folders` - Organização de workspace
- `ultra_core_folder_shares` - Compartilhamento de pastas
- `ultra_core_mcp_servers` - Servidores Model Context Protocol
...
const agentApi = axios.create({
  baseURL: `${import.meta.env.VITE_ULTRA_AI_CORE_SERVICE_URL}/api/v1`,
});
...
Desenvolvido com ❤️ para a comunidade Ultra CRM