...
type Agent struct {
	...
	ApiKeyID         *uuid.UUID `json:"api_key_id" gorm:"type:uuid;references:ultra_core_api_keys(id)"`
	Instruction      string     `json:"instruction" gorm:"type:text"`
	CardURL          string     `json:"card_url" gorm:"type:varchar(1024)"`
	FolderID         *uuid.UUID `json:"folder_id" gorm:"type:uuid;references:ultra_core_folders(id)"`
	...
	UltralutionBotID   *uuid.UUID `json:"ultralution_bot_id" gorm:"type:uuid;"`                    // Internal field - managed by ultralutionService only, not from frontend requests
	UltralutionBotSync bool       `json:"ultralution_bot_sync" gorm:"type:boolean;default:false;"` // Internal field - managed by ultralutionService only, not from frontend requests
...
type AgentResponse struct {
	...
	UltralutionBotID   *uuid.UUID             `json:"ultralution_bot_id"`
	UltralutionBotSync bool                   `json:"ultralution_bot_sync"`
...
func (a *Agent) ToResponse(aiProcessorURL string) *AgentResponse {
	return &AgentResponse{
		...
		Role:             a.Role,
		Goal:             a.Goal,
		UltralutionBotID:   a.UltralutionBotID,
		UltralutionBotSync: a.UltralutionBotSync,
