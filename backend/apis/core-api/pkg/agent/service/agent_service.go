...
import (
	...
	errorsPostgres "ultra-ai-core-service/internal/infra/postgres"
	"ultra-ai-core-service/internal/utils/stringutils"
	"ultra-ai-core-service/pkg/agent/client/a2a"
	"ultra-ai-core-service/pkg/agent/model"
	"ultra-ai-core-service/pkg/agent/repository"
	"ultra-ai-core-service/pkg/agent/service/processor"
	"ultra-ai-core-service/pkg/agent/service/validator"
	"ultra-ai-core-service/pkg/agent/utils"
	apiKeyService "ultra-ai-core-service/pkg/api_key/service"
	customMCPServer "ultra-ai-core-service/pkg/custom_mcp_server/service"
	customToolModel "ultra-ai-core-service/pkg/custom_tool/model"
	customTool "ultra-ai-core-service/pkg/custom_tool/service"
	folderService "ultra-ai-core-service/pkg/folder/service"
	mcpmodel "ultra-ai-core-service/pkg/mcp_server/model"
...
type agentService struct {
	...
	ultralutionService       ultralutionService
...
func (s *agentService) Create(ctx context.Context, request model.Agent) (*model.Agent, error) {
	...
	// Create ultralution bot synchronously to ensure it works with the current context
	ultralutionBot, err := s.ultralutionService.CreateAgentBot(ctx, agent, s.aiProcessorURL)
	if err != nil {
		log.Printf("Failed to create ultralution bot for agent %s: %v", agent.ID, err)
		// Don't fail the agent creation if bot creation fails
		// The user can manually sync later using the sync endpoint
	} else {
		agent.UltralutionBotID = &ultralutionBot.ID
		agent.UltralutionBotSync = true
		agent, err = s.agentRepository.Update(ctx, agent, agent.ID)
		if err != nil {
			s.ultralutionService.CleanupultralutionBot(ctx, ultralutionBot.ID)
			return nil, errorsPostgres.MapDBError(err, model.AgentErrors)
		}
	}
...
func (s *agentService) Update(ctx context.Context, request *model.Agent, id uuid.UUID) (*model.Agent, error) {
	...
	// Update ultralution bot synchronously to ensure it works with the current context
	ultralutionBot, err := s.ultralutionService.UpdateAgentBot(ctx, agent, s.aiProcessorURL)
	if err != nil {
		log.Printf("Failed to update ultralution bot for agent %s: %v", agent.ID, err)
...
func (s *agentService) Delete(ctx context.Context, id uuid.UUID) (bool, error) {
	...
	ultralutionBotID := agent.UltralutionBotID
	ultralutionBotSync := agent.UltralutionBotSync

	// PRIMEIRO: Deletar bot externo (antes de deletar agent)
	if ultralutionBotID != nil && ultralutionBotSync {
		log.Printf("Deleting ultralution bot %s for agent %s", *ultralutionBotID, agent.ID)

		if err := s.ultralutionService.DeleteAgentBot(ctx, agent); err != nil {
			log.Printf("Failed to delete ultralution bot %s for agent %s: %v, attempting safe deletion",
				*ultralutionBotID, agent.ID, err)

			// Tentar safe delete como fallback
			if safeErr := s.ultralutionService.DeleteAgentBotSafe(ctx, agent); safeErr != nil {
				log.Printf("WARNING: Bot %s may be orphaned after agent %s deletion: %v",
					*ultralutionBotID, agent.ID, safeErr)
			} else {
				log.Printf("Successfully deleted ultralution bot %s using safe mode", *ultralutionBotID)
			}
		} else {
			log.Printf("Successfully deleted ultralution bot %s for agent %s", *ultralutionBotID, agent.ID)
		}
	}
	...
	if request.FolderID != "" {
		...
		for i, agentResp := range agentsResponse.Items {
			...
			agent := &model.Agent{
				...
				Role:             agentResp.Role,
				Goal:             agentResp.Goal,
				UltralutionBotID:   agentResp.UltralutionBotID,
				UltralutionBotSync: agentResp.UltralutionBotSync,
