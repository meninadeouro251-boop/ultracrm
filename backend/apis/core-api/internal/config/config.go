...
type Config struct {
	...
	JWT                JWTConfig
	DB                 DBConfig
	UltraCRM          UltraCRMConfig
	UltraAuth        UltraAuthConfig
...
// JWTConfig for validating ultralution JWT tokens
type JWTConfig struct {
	SecretKey string // Same secret key used by ultralution for token validation
	Algorithm string // Algorithm used by ultralution (usually HS256)
}
...
// UltraCRMConfig holds configuration for Ultra CRM integration
type UltraCRMConfig struct {
	BaseURL string
}

// UltraAuthConfig holds configuration for Ultra Auth service integration
type UltraAuthConfig struct {
	BaseURL string
}
...
func Load() (*Config, error) {
	return &Config{
		Core: CoreConfig{
			...
			EncryptionKey: LoadEnvOrPanic("ENCRYPTION_KEY"), // Shared with ultra-ai-processor
		},
		JWT: JWTConfig{
			SecretKey: LoadEnvOrPanic("JWT_SECRET_KEY"), // Same secret as ultralution
		...
		UltraCRM: UltraCRMConfig{
			BaseURL: LoadEnvOrDefault("ULTRA_CRM_BASE_URL", ""),
		},
		UltraAuth: UltraAuthConfig{
			BaseURL: LoadEnvOrPanic("ULTRA_AUTH_BASE_URL"),
		},
		AIProcessorService: AIProcessorServiceConfig{
