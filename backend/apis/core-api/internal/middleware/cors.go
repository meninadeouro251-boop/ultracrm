...
func CORSConfig(ultralutionBaseURL string) *corsConfig {
	allowedOrigins := []string{
		"http://localhost:5173",
		"http://127.0.0.1:5173",
	}

	// Add ultralution base URL if not empty
	if ultralutionBaseURL != "" {
		allowedOrigins = append(allowedOrigins, ultralutionBaseURL)
	}
	...
	if ultracloudURL := os.Getenv("ultraCLOUD_FRONTEND_URL"); ultracloudURL != "" {
		allowedOrigins = append(allowedOrigins, ultracloudURL)
	}
