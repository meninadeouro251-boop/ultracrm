...
func GetUser(ctx context.Context) (types.UltraAuthUser, error) {
	if user, ok := ctx.Value("user").(types.UltraAuthUser); ok {
		return user, nil
	}

	return types.UltraAuthUser{}, errors.New("user not found in context")
}
