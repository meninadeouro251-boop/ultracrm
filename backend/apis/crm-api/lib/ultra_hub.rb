...
class UltraHub
  BASE_URL = ENV.fetch('ULTRA_HUB_URL', 'https://localhost:9999')
  ...
  def self.support_config
    {
      support_website_token: InstallationConfig.find_by(name: 'ULTRA_SUPPORT_WEBSITE_TOKEN')&.value,
      support_script_url: InstallationConfig.find_by(name: 'ULTRA_SUPPORT_SCRIPT_URL')&.value,
      support_identifier_hash: InstallationConfig.find_by(name: 'ULTRA_SUPPORT_IDENTIFIER_HASH')&.value
    }
  end
  ...
  def self.instance_config
    {
      ...
      installation_version: Ultra.config[:version],
      installation_host: URI.parse(ENV.fetch('FRONTEND_URL', '')).host,
      installation_env: ENV.fetch('INSTALLATION_ENV', ''),
      edition: ENV.fetch('ULTRA_EDITION', '')
    }
  end
  ...
  def self.sync_with_hub
    ...
    return { 'version' => Ultra.config[:version] }
  end
