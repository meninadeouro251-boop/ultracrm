...
class ContactInboxWithContactBuilder
  ...
  def find_or_create_contact_and_contact_inbox
    # For non-ultralution Go channels, use the simple source_id lookup
    unless ultralution_go_channel?
    ...
    # For ultralution Go, do a smart search first
    if ultralution_go_channel?
      # Try to find existing ContactInbox or reuse one from the same contact
      Rails.logger.info "ultralution Go: Smart contact/inbox lookup for source_id: #{source_id}"
      perform_ultralution_go_lookup
      return @contact_inbox if @contact_inbox
    end
  ...
  def ultralution_go_channel?
    inbox.channel_type == 'Channel::Whatsapp' && inbox.channel.provider == 'ultralution_go'
  end

  private

  def perform_ultralution_go_lookup
    # First check if ContactInbox with this exact source_id already exists
    @contact_inbox = inbox.contact_inboxes.find_by(source_id: source_id) if source_id.present?
    if @contact_inbox
      Rails.logger.info "ultralution Go: Found existing ContactInbox #{@contact_inbox.id} with exact source_id '#{source_id}'"
    ...
    if @contact
      ...
      existing_contact_inbox = find_existing_contact_inbox_for_ultralution_go(@contact)
      if existing_contact_inbox
        Rails.logger.info "ultralution Go: Contact #{@contact.id} already has ContactInbox #{existing_contact_inbox.id} (source_id: '#{existing_contact_inbox.source_id}')"
        Rails.logger.info "ultralution Go: Updating to new source_id '#{source_id}' - REUSING existing ContactInbox"
    ...
    Rails.logger.info "ultralution Go: No existing ContactInbox found for source_id '#{source_id}' - will create new contact/inbox"
  end
  ...
  def build_contact_with_contact_inbox
    ...
    # Only create contact_inbox if not already found in find_contact (for ultralution Go)
  ...
  def update_contact_avatar(contact)
    if contact_attributes[:avatar_url]
      ...
      # Use ultralution Go API for WhatsApp Cloud avatar fetching if configured in Admin settings
  ...
  def find_existing_contact_inbox_for_ultralution_go(contact)
