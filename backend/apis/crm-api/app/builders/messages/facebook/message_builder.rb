# This class creates both outgoing messages from ultralution and echo outgoing messages based on the flag `outgoing_echo`
# Assumptions
# 1. Incase of an outgoing message which is echo, source_id will NOT be nil,
#    based on this we are showing "not sent from ultralution" message in frontend
#    Hence there is no need to set user_id in message for outgoing echo messages.

# Load ultralutionExceptionTracker (using load to avoid Zeitwerk constant name mismatch)
load Rails.root.join('lib', 'ultralution_exception_tracker.rb').to_s unless defined?(ultralutionExceptionTracker)
...
class Messages::Facebook::MessageBuilder < Messages::Messenger::MessageBuilder
  ...
  def perform
    ...
    ultralutionExceptionTracker.new(e, account: nil).capture_exception
    true
  end
  ...
  def contact_params
    begin
      ...
      if e.message.include?('2018218')
        ...
        ultralutionExceptionTracker.new(e, account: nil).capture_exception unless @outgoing_echo
      end
    rescue StandardError => e
      result = {}
      ultralutionExceptionTracker.new(e, account: nil).capture_exception
