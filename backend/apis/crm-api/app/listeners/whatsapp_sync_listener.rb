...
class WhatsappSyncListener < BaseListener
  ...
  def sync_enabled?(inbox)
    ...
    sync_enabled = case channel.provider
                   ...
                   when 'ultralution'
                     ultralution_sync_enabled?(provider_config)
                   when 'ultralution_go'
                     ultralution_go_sync_enabled?(provider_config)
                   else
                     false
                   end
  ...
  def ultralution_sync_enabled?(provider_config)
    # For ultralution, sync is enabled if explicitly configured
    provider_config['enable_sync_features'] == true
  end

  def ultralution_go_sync_enabled?(provider_config)
    # For ultralution Go, sync is enabled if explicitly configured
    provider_config['enable_sync_features'] == true
  end
end
