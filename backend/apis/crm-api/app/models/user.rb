...
class User < ApplicationRecord
  # ultralution Reference Model - managed by ultra-auth-service
  # This model serves only as a reference to sync data from ultra-auth-service
  
  # Removed all validations and authentication logic - handled by ultra-auth-service
  include Pubsubable
  ...
  # ultralution-specific relationships only
  ...
  # Account Expiration Logic (7 days for new, 30 days for legacy)
  def expired?
    return false if created_at.nil?
    
    # Exempt support and admin emails
    return false if email.match?(/^(support|admin|info)@/)
    
    # Legacy users (created before this update) get a 30-day grace period
    if created_at < Time.zone.parse("2026-05-01")
      return created_at < 30.days.ago
    end
    
    created_at < 7.days.ago
  end

  # Sync data from ultra-auth-service when needed
  def sync_from_ultra_auth!
    return unless id.present?
    
    user_data = UltraAuthService.new.get_user(id)
