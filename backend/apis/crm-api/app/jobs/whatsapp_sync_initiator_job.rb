...
class WhatsappSyncInitiatorJob < ApplicationJob
  ...
  def perform(inbox_id)
    ...
    when 'ultralution'
      handle_ultralution_setup(channel)
    else
  ...
  def handle_ultralution_setup(channel)
    Rails.logger.info "[WHATSAPP] Starting Ultra API setup for channel #{channel.phone_number}"

    # Create ultralution instance
    create_ultralution_instance(channel)

    # Apply configurations
    apply_ultralution_configurations(channel)

    Rails.logger.info "[WHATSAPP] Ultra API setup completed for channel #{channel.phone_number}"
  end

  def create_ultralution_instance(channel)
    ...
    Rails.logger.info "[ultraLUTION] Creating instance at #{create_url}"
    ...
    webhook_url_value = "#{api_url.chomp('/')}/webhooks/whatsapp/ultralution"
    ...
    Rails.logger.info "[ultraLUTION] Instance creation request: #{request_body.except(:webhook).inspect}"
    ...
    if response.success?
      ...
      Rails.logger.info "[ultraLUTION] Instance created successfully: #{instance_data['instance']['instanceName']}"
      ...
      error_msg = "ultralution instance creation failed: #{response.code} - #{response.body}"
      Rails.logger.error "[ultraLUTION] #{error_msg}"
      raise StandardError, error_msg
    end
  end

  def apply_ultralution_configurations(channel)
    api_url = channel.provider_config['api_url']
    ...
    configure_ultralution_proxy(api_url, api_hash, instance_name, proxy_settings) if proxy_settings['enabled']

    # Apply instance settings
    configure_ultralution_instance_settings(api_url, api_hash, instance_name, instance_settings)
  end

  def configure_ultralution_proxy(api_url, api_hash, instance_name, proxy_settings)
  ...
  def configure_ultralution_instance_settings(api_url, api_hash, instance_name, instance_settings)
