class Webhooks::WhatsappEventsJob < ApplicationJob
  ...
  def sync_event?(params)
    ...
    # Ultra API sync events
    ultralution_event = params[:event]
    ultralution_sync_events = %w[contacts.upsert messages.set]

    is_whatsapp_cloud_sync = whatsapp_cloud_sync_fields.include?(whatsapp_cloud_field)
    is_ultralution_sync = ultralution_sync_events.include?(ultralution_event)

    is_sync = is_whatsapp_cloud_sync || is_ultralution_sync

    Rails.logger.info "Sync event detection: whatsapp_field=#{whatsapp_cloud_field}, ultralution_event=#{ultralution_event}, is_sync=#{is_sync}"
    is_sync
  end
  ...
  def handle_sync_events(channel, params)
    ...
    when 'ultralution'
      handle_ultralution_sync_events(channel, params)
    else
  ...
  def handle_ultralution_sync_events(channel, params)
    event = params[:event]
    Rails.logger.info "Processing ultralution sync event: #{event} for channel #{channel.phone_number}"

    case event
    when 'contacts.upsert'
      handle_ultralution_contacts_sync(channel, params)
    when 'messages.set'
      handle_ultralution_messages_sync(channel, params)
    else
      Rails.logger.warn "Unknown ultralution sync event: #{event}"
    end
  end

  def handle_ultralution_contacts_sync(channel, params)
    contacts_data = params[:data]
    return unless contacts_data.is_a?(Array)

    Rails.logger.info "[ultraLUTION] Processing #{contacts_data.size} contacts from sync webhook"

    contacts_data.each do |contact_data|
      process_ultralution_contact(channel, contact_data)
    end
  end

  def process_ultralution_contact(channel, contact_data)
    ...
    # Create or update contact in ultralution
    ::ContactInboxWithContactBuilder.new(
      ...
      contact_attributes: {
        ...
        additional_attributes: {
          ultralution_contact_synced: true,
          ultralution_sync_timestamp: Time.current.to_i,
          ultralution_profile_pic_url: contact_data['profilePicUrl'],
          ultralution_instance_id: contact_data['instanceId']
        }
      }
    ).perform

    Rails.logger.info "[ultraLUTION] Contact synced via webhook: #{push_name} (#{formatted_phone})"
  rescue StandardError => e
    Rails.logger.error "[ultraLUTION] Contact sync failed for #{remote_jid}: #{e.message}"
  end

  def handle_ultralution_messages_sync(channel, params)
    messages_data = params[:data]
    return unless messages_data.is_a?(Array)

    Rails.logger.info "[ultraLUTION] Processing #{messages_data.size} messages from sync webhook"
    ...
    messages_by_chat.each do |remote_jid, chat_messages|
      process_ultralution_conversation_sync(channel, remote_jid, chat_messages)
    end
  end

  def process_ultralution_conversation_sync(channel, remote_jid, messages)
    ...
    messages.each do |message_data|
      process_ultralution_sync_message(channel, conversation, message_data)
    end

    Rails.logger.info "[ultraLUTION] Conversation sync processed: #{conversation.id} with #{messages.size} messages"
  rescue StandardError => e
    Rails.logger.error "[ultraLUTION] Conversation sync failed for #{remote_jid}: #{e.message}"
  end

  def process_ultralution_sync_message(channel, conversation, message_data)
    ...
    content = extract_ultralution_sync_message_content(message_data)
    ...
    # Create message in ultralution with original timestamp
    message = conversation.messages.create!(
      ...
      content_attributes: {
        ...
        ultralution_synced: true,
        ultralution_message_type: message_data['messageType'],
        ultralution_status: message_data['status']
      }
    )

    Rails.logger.info "[ultraLUTION] Sync message created: #{message.id} - #{content.truncate(50)}"
  rescue StandardError => e
    Rails.logger.error "[ultraLUTION] Sync message failed for #{message_id}: #{e.message}"
  end

  def extract_ultralution_sync_message_content(message_data)
  ...
  def handle_message_events(channel, params)
    ...
    when 'ultralution'
      Whatsapp::IncomingMessageultralutionService.new(inbox: channel.inbox, params: params).perform
    when 'ultralution_go'
      Whatsapp::IncomingMessageultralutionGoService.new(inbox: channel.inbox, params: params).perform
    when 'notificame'
  ...
  def try_find_channel_by_phone_number(params)
    ...
    # For ultralution Go, prioritize finding by instanceId
    if params[:instanceId].present?
      channel = find_channel_by_ultralution_go_instance(params[:instanceId])
      if channel
        Rails.logger.info "Channel search via ultralution Go instanceId #{params[:instanceId]}: found #{channel.phone_number}"
        return channel
      end
    end

    # For Ultra API, prioritize finding by instance name + server_url
    if params[:instance].present? && params[:event].present?
      channel = find_channel_by_ultralution_instance(params[:instance], params[:server_url])
      if channel
        Rails.logger.info "Channel search via ultralution instance #{params[:instance]}: found #{channel.phone_number}"
        return channel
      end
    end
  ...
  def find_channel_by_ultralution_instance(instance_name, server_url = nil)
    ...
    if server_url.present?
      channel = Channel::Whatsapp.joins(:inbox)
                                 .where(provider: 'ultralution')
                                 .where("provider_config ->> 'instance_name' = ?", instance_name)
                                 .where("provider_config ->> 'api_url' = ?", server_url)
                                 .first

      Rails.logger.info "ultralution channel search: instance=#{instance_name}, server_url=#{server_url} - #{channel ? 'found' : 'not found'}"
    ...
    Rails.logger.info "ultralution channel search (fallback): instance=#{instance_name} only - #{channel ? 'found' : 'not found'}"
    channel
  end
  ...
  def find_channel_by_ultralution_go_instance(instance_uuid)
    Rails.logger.info "ultralution Go channel search: Searching for instance_uuid=#{instance_uuid}"

    # List all ultralution Go channels for debugging
    all_ultralution_go_channels = Channel::Whatsapp.joins(:inbox)
                                                 .where(provider: 'ultralution_go')

    Rails.logger.info "ultralution Go channel search: Found #{all_ultralution_go_channels.count} ultralution_go channels total"

    all_ultralution_go_channels.each do |channel|
      config_instance_uuid = channel.provider_config['instance_uuid']
      Rails.logger.info "ultralution Go channel search: Channel #{channel.id} has instance_uuid: #{config_instance_uuid}"
    end
    ...
    Rails.logger.info "ultralution Go channel search: instance_uuid=#{instance_uuid} - #{channel ? "found channel #{channel.id}" : 'not found'}"
    channel
  end
  ...
  def log_channel_search_result(channel, params)
    if channel
      ...
      # Additional debugging for Ultra API
      if params[:instance].present?
        ultralution_channels = Channel::Whatsapp.where(provider: %w[ultralution ultralution_go])
        Rails.logger.warn "Available ultralution channels: #{ultralution_channels.map do |c|
