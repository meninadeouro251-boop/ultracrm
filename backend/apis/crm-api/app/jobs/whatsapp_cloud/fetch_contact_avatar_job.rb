class WhatsappCloud::FetchContactAvatarJob < ApplicationJob
  ...
  def perform(contact_id, phone_number)
    ...
    # Check if ultralution Go is configured in Admin settings
    return unless ultralution_go_configured?

    phone_without_plus = phone_number.gsub(/^\+/, '')
    profile_picture_url = fetch_profile_picture_from_ultralution_go(phone_without_plus)

    if profile_picture_url.present?
      Avatar::AvatarFromUrlJob.perform_later(contact, profile_picture_url)
      Rails.logger.info "WhatsApp Cloud: ultralution Go profile picture scheduled for contact #{contact.id}"
    end
  ...
  def ultralution_go_configured?
    api_url = GlobalConfigService.load('ultraLUTION_GO_API_URL', '')
    instance_token = GlobalConfigService.load('ultraLUTION_GO_INSTANCE_SECRET', '')

    api_url.present? && instance_token.present?
  end

  def fetch_profile_picture_from_ultralution_go(phone_number)
    api_url = GlobalConfigService.load('ultraLUTION_GO_API_URL', '')
    instance_token = GlobalConfigService.load('ultraLUTION_GO_INSTANCE_SECRET', '')

    Rails.logger.debug { "WhatsApp Cloud: Calling ultralution Go /user/avatar for number #{phone_number}" }
    ...
    if profile_picture_url.present?
      Rails.logger.info 'WhatsApp Cloud: Profile picture URL retrieved via ultralution Go'
      profile_picture_url
    else
