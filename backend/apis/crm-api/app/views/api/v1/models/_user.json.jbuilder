...
json.hmac_identifier resource.hmac_identifier if GlobalConfig.get('ultraLUTION_INBOX_HMAC_KEY')['ultraLUTION_INBOX_HMAC_KEY'].present?
json.id resource.id
json.inviter_id nil
...
json.accounts do
  json.array! [resource] do |user|
    json.id 1
    json.name GlobalConfig.get('BRAND_NAME')['BRAND_NAME'] || 'ultra CRM'
    json.status 'active'
