module Whatsapp::ultralutionGoHandlers::ProfilePictureHandler
  ...
  def update_contact_profile_picture(contact, phone_number)
    ...
    Rails.logger.info "ultralution Go API: Scheduling avatar fetch for contact #{contact.id} (primary: #{primary_number}, fallback: #{fallback_number})"

    # Use global configuration if available, otherwise fall back to channel config
    if global_ultralution_go_configured?
      Rails.logger.info "ultralution Go API: Using global configuration for avatar fetch"
      ultralutionGo::FetchContactAvatarWithFallbackJob.perform_later(
        contact.id,
        primary_number,
        fallback_number,
        global_api_base_url,
      ...
      Rails.logger.info "ultralution Go API: Using channel configuration for avatar fetch"
      ultralutionGo::FetchContactAvatarWithFallbackJob.perform_later(
        contact.id,
        primary_number,
        fallback_number,
    ...
    Rails.logger.error "ultralution Go API: Failed to schedule avatar fetch for contact #{contact.id}: #{e.message}"
  end
  ...
  def determine_avatar_fetch_numbers(contact, phone_number)
    ...
    if contact.identifier.present?
      ...
      Rails.logger.info "ultralution Go API: Using identifier '#{primary}' as primary, phone '#{fallback}' as fallback"
    else
      # No identifier available, use phone_number as primary only
      primary = phone_number
      fallback = nil
      Rails.logger.info "ultralution Go API: Using phone '#{primary}' as primary (no identifier available)"
    end
  ...
  def should_fetch_profile_picture?
    ...
    global_ultralution_go_configured? || channel_ultralution_go_configured?
  end

  def global_ultralution_go_configured?
    global_api_base_url.present? && global_instance_token.present?
  end

  def channel_ultralution_go_configured?
    api_base_url.present? && instance_token.present?
  end

  # Global configuration from Admin settings
  def global_api_base_url
    @global_api_base_url ||= GlobalConfigService.load('ultraLUTION_GO_API_URL', '')
  end

  def global_instance_token
    @global_instance_token ||= GlobalConfigService.load('ultraLUTION_GO_INSTANCE_SECRET', '')
  end
