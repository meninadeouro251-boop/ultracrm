...
module Whatsapp::ultralutionGoHandlers::MessagesUpsert
  include Whatsapp::ultralutionGoHandlers::Helpers
  include Whatsapp::ultralutionGoHandlers::ProfilePictureHandler
  ...
  def handle_message
    ...
    Rails.logger.info "ultralution Go API: Creating new message #{raw_message_id}"
  ...
  def set_contact
    Rails.logger.info "ultralution Go API: Setting contact - inbox present: #{inbox.present?}"
    Rails.logger.info "ultralution Go API: Inbox details: #{inbox.inspect}" if inbox
    ...
    Rails.logger.info "ultralution Go API: Creating contact with source_id: #{source_id}, phone_number: #{phone_number}, push_name: #{push_name}, sender_alt: #{sender_alt_value}, is_whatsapp: #{is_whatsapp_number}"
    ...
    Rails.logger.info "ultralution Go API: Contact set - ID: #{@contact.id}, Name: #{@contact.name}, Identifier: #{@contact.identifier}, Source ID: #{@contact_inbox.source_id}"
  end

  def determine_source_id(sender_alt_value, phone_number)
    if sender_alt_value.present?
      Rails.logger.info "ultralution Go API: Using SenderAlt '#{sender_alt_value}' as source_id"
      sender_alt_value
    else
      Rails.logger.info "ultralution Go API: Using phone_number '#{phone_number}' as source_id (no SenderAlt available)"
      phone_number
    end
  end
  ...
  def update_contact_information(push_name, phone_number, sender_alt_value, is_whatsapp_number)
    ...
    if @contact.identifier.blank? && sender_alt_value.present?
      ...
      Rails.logger.info "ultralution Go API: Adding identifier #{sender_alt_value} to existing contact #{@contact.id}"
    end
    ...
    if @contact.phone_number.blank? && is_whatsapp_number
      ...
      Rails.logger.info "ultralution Go API: Adding phone_number +#{phone_number} to contact #{@contact.id}"
    end
  ...
  def create_message(attach_media: false)
    Rails.logger.info "ultralution Go API: Creating message with content: #{message_content}"
    Rails.logger.info "ultralution Go API: Attach media flag: #{attach_media}"

    # Check if this is a quoted/reply message
    reply_to_id = quoted_message_id
    is_quoted = quoted_message?

    Rails.logger.info "ultralution Go API: Is quoted message? #{is_quoted}"
    Rails.logger.info "ultralution Go API: Message is a reply to: #{reply_to_id}" if reply_to_id.present?
    Rails.logger.info 'ultralution Go API: No reply ID found for quoted message' if is_quoted && reply_to_id.blank?

    # Find or create conversation
    conversation = find_or_create_conversation

    # Build message attributes (like ultralution v2)
    ...
    Rails.logger.info "ultralution Go API: Message saved - ID: #{@message.id}, Reply attributes: #{@message.content_attributes.slice(:in_reply_to,
                                                                                                                                   :in_reply_to_external_id)}"
    Rails.logger.info "ultralution Go API: Message created successfully - ID: #{@message.id}, Content: #{@message.content&.truncate(100)}"

    # Notify like ultralution v2
  ...
  def build_message_attributes(conversation, reply_to_id = nil)
    ...
    if reply_to_id.present?
      ...
      Rails.logger.info "ultralution Go API: Adding reply reference to content_attributes: #{reply_to_id}"
    end
    ...
    Rails.logger.info "ultralution Go API: Message built with attributes: #{message_attributes.keys}"
    Rails.logger.info "ultralution Go API: Message content_attributes: #{@message.content_attributes}"
  end

  def handle_attach_media
    Rails.logger.info "ultralution Go API: Processing attachment for message #{raw_message_id}, type: #{message_type}"
    ...
    Rails.logger.error "ultralution Go API: Failed to download attachment for message #{raw_message_id}: #{e.message}"
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: Failed to create attachment for message #{raw_message_id}: #{e.message}"
  ...
  def save_message_and_notify
    ...
    Rails.logger.info "ultralution Go API: Message created successfully - ID: #{@message.id}, Content: #{@message.content&.truncate(100)}"
    Rails.logger.info "ultralution Go API: Final reply attributes: #{@message.content_attributes.slice(:in_reply_to, :in_reply_to_external_id)}"
  ...
  def find_or_create_conversation
    ...
    if conversation.blank?
      Rails.logger.info "ultralution Go API: Creating new conversation for contact #{@contact.id}"
      ...
      conversation = ::Conversation.create!(
        ...
        additional_attributes: {
          ultralution_go_chat_id: conversation_id
        }
      )
    else
      Rails.logger.info "ultralution Go API: Using existing conversation #{conversation.id}"
    end
  ...
  def attach_media_from_url(media_url)
    ...
    Rails.logger.info 'ultralution Go API: Media attachment debug info:'
    ...
    if response.success?
      Rails.logger.info 'ultralution Go API: File download successful:'
      ...
      Rails.logger.info 'ultralution Go API: Attachment created successfully:'
      ...
      Rails.logger.error "ultralution Go API: Failed to download media: #{response.code} - #{response.message}"
    end
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: Media attachment error: #{e.message}"
    Rails.logger.error e.backtrace.first(5).join('\n')
  end

  def media_message?
    # ultralution Go: Check if it's a media message
    return false unless @ultralution_go_info

    media_type = @ultralution_go_info[:MediaType]
    message_type = @ultralution_go_info[:Type]

    # Media types in ultralution Go: image, video, audio, document
    media_type.present? || message_type == 'media'
  end

  def message_content
    # ultralution Go: Extract content based on message type
    return nil unless @ultralution_go_message

    message = @ultralution_go_message
  ...
  def extract_media_url
    # ultralution Go provides processed mediaUrl directly at Message level
    message = @ultralution_go_message
    Rails.logger.info 'ultralution Go API: Extracting media URL from message'
    Rails.logger.info "ultralution Go API: Message structure for media: #{message&.keys}"
    return nil unless message

    # ultralution Go structure has mediaUrl at the root Message level
    media_url = message[:mediaUrl]
    Rails.logger.info "ultralution Go API: Root level mediaUrl: #{media_url}"

    # Fallback: check inside specific message types if not found at root
    if media_url.blank?
      Rails.logger.info 'ultralution Go API: Checking inside specific message types for mediaUrl'
      ...
      Rails.logger.info "ultralution Go API: imageMessage.mediaUrl: #{image_url}"
      Rails.logger.info "ultralution Go API: videoMessage.mediaUrl: #{video_url}"
      Rails.logger.info "ultralution Go API: audioMessage.mediaUrl: #{audio_url}"
      Rails.logger.info "ultralution Go API: documentMessage.mediaUrl: #{doc_url}"
      Rails.logger.info "ultralution Go API: stickerMessage.mediaUrl: #{sticker_url}"

      media_url = image_url || video_url || audio_url || doc_url || sticker_url
    end

    Rails.logger.info "ultralution Go API: Final extracted media URL: #{media_url}"
    media_url
  end
  ...
  def determine_content_type
    return 'text/plain' unless @ultralution_go_message
  ...
  def determine_file_extension
    # ultralution Go: Extract extension from MediaType
    media_type = @ultralution_go_info[:MediaType]
    ...
    when 'video'
      '.mp4'
    when 'audio', 'ptt'  # PTT is audio in ultralution Go
  ...
  def file_content_type
    # EXACTLY like ultralution v2: based on message type, not MediaType
  ...
  def message_type_from_media
    # Convert ultralution Go MediaType to message_type like ultralution v2
    media_type = @ultralution_go_info[:MediaType]
    ...
    when 'video'
      'video'
    when 'audio', 'ptt'  # PTT (push-to-talk) is audio in ultralution v2
      'audio'
    when 'document'
      'file'
    else
  ...
  def configure_audio_metadata(attachment)
    ...
    audio_message = @ultralution_go_message['audioMessage']
    ...
    if audio_message['waveform'].present?
      ...
      Rails.logger.info "ultralution Go API: Audio waveform extracted (#{audio_message['waveform'].length} chars)"
    end
  ...
  def audio_voice_note?
    # ultralution Go: Check if it's a PTT voice note (like ultralution v2)
    media_type = @ultralution_go_info[:MediaType]

    # Voice notes in ultralution Go have MediaType 'ptt'
    media_type == 'ptt'
  end
  ...
  def download_attachment_file
    ...
    if message[:mediaUrl].present?
      Rails.logger.info "ultralution Go API: Downloading from mediaUrl: #{message[:mediaUrl]}"
      return Down.download(message[:mediaUrl])
    end

    Rails.logger.warn 'ultralution Go API: No media found - no mediaUrl'
    nil
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: Failed to download media: #{e.message}"
    nil
  end

  def debug_media_info
    Rails.logger.info 'ultralution Go API: Media debug info:'
    Rails.logger.info "- Message type: #{message_type}"
    Rails.logger.info "- Media URL: #{@ultralution_go_message[:mediaUrl]}"
    Rails.logger.info "- Mimetype: #{message_mimetype}"
  end

  def log_attachment_info(attachment_file, filename, content_type)
    Rails.logger.info 'ultralution Go API: Creating attachment:'
  ...
  def log_attachment_success(attachment)
    Rails.logger.info "ultralution Go API: Attachment created successfully with ID: #{attachment.id}"
  end

  def configure_audio_metadata(attachment)
    attachment.meta = { is_recorded_audio: true } if message_type == 'audio' && @ultralution_go_message.dig(:audioMessage, :ptt)
  end

  def audio_voice_note?
    message_type == 'audio' && @ultralution_go_message.dig(:audioMessage, :ptt)
  end
  ...
  def document_extension
    filename_from_message = @ultralution_go_message.dig(:documentMessage, :fileName) ||
                            @ultralution_go_message.dig(:documentWithCaptionMessage, :message, :documentMessage, :fileName)
  ...
  def filename
    filename = @ultralution_go_message.dig(:documentMessage, :fileName) ||
               @ultralution_go_message.dig(:documentWithCaptionMessage, :message, :documentMessage, :fileName)
  ...
  def message_mimetype
    case message_type
    when 'image'
      @ultralution_go_message.dig(:imageMessage, :mimetype)
    when 'sticker'
      @ultralution_go_message.dig(:stickerMessage, :mimetype)
    when 'video'
      @ultralution_go_message.dig(:videoMessage, :mimetype)
    when 'audio'
      @ultralution_go_message.dig(:audioMessage, :mimetype)
    when 'file'
      @ultralution_go_message.dig(:documentMessage, :mimetype) ||
        @ultralution_go_message.dig(:documentWithCaptionMessage, :message, :documentMessage, :mimetype)
    end
  end
end
