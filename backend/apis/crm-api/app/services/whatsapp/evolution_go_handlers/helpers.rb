module Whatsapp::ultralutionGoHandlers::Helpers
  private
  ...
  # For ultralution Go, it's the Chat field from Info
  def conversation_id
  ...
  def sender_alt
    @ultralution_go_info&.dig(:SenderAlt)
  end
  ...
  def message_timestamp
    # ultralution Go: Parse timestamp from Info
    timestamp_str = @ultralution_go_info&.dig(:Timestamp)
    return Time.current.to_i unless timestamp_str

    begin
      Time.parse(timestamp_str).to_i
    rescue ArgumentError
      Rails.logger.warn "ultralution Go API: Invalid timestamp format: #{timestamp_str}"
      Time.current.to_i
    end
  end

  def message_content
    # ultralution Go: Extract content from Message object
  ...
  def message_type
    # ultralution Go: Use Type from Info
    @ultralution_go_info&.dig(:Type)&.downcase
  end

  def message_processable?
    Rails.logger.info 'ultralution Go API: Checking if message is processable'
    Rails.logger.info "ultralution Go API: Message ID: #{raw_message_id}"
    Rails.logger.info "ultralution Go API: Is incoming: #{incoming?}"

    return false if raw_message_id.blank?
    return false unless message_content.present? || @ultralution_go_message.present?

    true
  end

  def quoted_message_id
    # ultralution Go: Extract quoted message ID from different possible locations
    message = @ultralution_go_message
    data = @ultralution_go_data

    Rails.logger.info 'ultralution Go API: Checking for quoted message'
    Rails.logger.info "ultralution Go API: Message structure: #{message&.keys}"
    Rails.logger.info "ultralution Go API: Data structure: #{data&.keys}"
    Rails.logger.info "ultralution Go API: Data isQuoted: #{data&.dig(:isQuoted)}"
    Rails.logger.info "ultralution Go API: Has extendedTextMessage: #{message&.dig(:extendedTextMessage).present?}"
    Rails.logger.info "ultralution Go API: extendedTextMessage keys: #{message&.dig(:extendedTextMessage)&.keys}"
    Rails.logger.info "ultralution Go API: contextInfo keys: #{message&.dig(:extendedTextMessage, :contextInfo)&.keys}"
    ...
    if quoted_id.present?
      Rails.logger.info "ultralution Go API: Found quoted ID in context.id: #{quoted_id}"
      return quoted_id
    end
    ...
    if quoted_id.present?
      Rails.logger.info "ultralution Go API: Found quoted ID in extendedTextMessage.contextInfo.stanzaId: #{quoted_id}"
      return quoted_id
    end
    ...
    if quoted_id.present?
      Rails.logger.info "ultralution Go API: Found quoted ID in root contextInfo.stanzaId: #{quoted_id}"
      return quoted_id
    end
    ...
    if quoted_id.present?
      Rails.logger.info "ultralution Go API: Found quoted ID in messageContextInfo.stanzaId: #{quoted_id}"
      return quoted_id
    end
    ...
    if quoted_id.present?
      Rails.logger.info "ultralution Go API: Found quoted ID in root quoted.stanzaID: #{quoted_id}"
      return quoted_id
    end
    ...
    %i[imageMessage videoMessage audioMessage documentMessage].each do |msg_type|
      ...
      if quoted_id.present?
        Rails.logger.info "ultralution Go API: Found quoted ID in #{msg_type}.contextInfo.stanzaId: #{quoted_id}"
        return quoted_id
      end
    ...
    if quoted_id.present?
      Rails.logger.info "ultralution Go API: Found quoted ID in contextInfo.quotedMessage.key.id: #{quoted_id}"
      return quoted_id
    end
    ...
    if quoted_id.present?
      Rails.logger.info "ultralution Go API: Found quoted ID in root quotedMessage.key.id: #{quoted_id}"
      return quoted_id
    end

    Rails.logger.info 'ultralution Go API: No quoted message ID found'
    nil
  end

  def quoted_message?
    # ultralution Go: Check if this is a quoted/reply message
    data = @ultralution_go_data
    message = @ultralution_go_message
  ...
  def has_media?
    # ultralution Go: Check if message has media content
    message = @ultralution_go_message
    Rails.logger.info 'ultralution Go API: Checking for media in message'
    Rails.logger.info "ultralution Go API: Message keys: #{message&.keys}"
    ...
    media_types.each do |type|
      ...
      Rails.logger.info "ultralution Go API: Has #{type}? #{has_type}"
      if has_type
        Rails.logger.info "ultralution Go API: #{type} content: #{message[type].keys if message[type].is_a?(Hash)}"
      end
    end

    has_media = media_types.any? { |type| message[type].present? }
    Rails.logger.info "ultralution Go API: Final has_media result: #{has_media}"
    has_media
  end

  def media_attachment?
    # ultralution Go: Check if message is a media type
    type = message_type
    %w[image video audio file sticker].include?(type)
  end

  def message_type
    # ultralution Go: Determine message type based on message structure
    message = @ultralution_go_message
