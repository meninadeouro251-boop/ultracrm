module Whatsapp::ultralutionGoHandlers::ReceiptHandler
  include Whatsapp::ultralutionGoHandlers::Helpers

  private

  def process_read_receipt
    Rails.logger.info 'ultralution Go API: Processing Receipt event'
    ...
    Rails.logger.info "ultralution Go API: Receipt state: #{event_state}"
    Rails.logger.debug { "ultralution Go API: Receipt data: #{receipt_data.inspect}" }

    # Extract message IDs and status
    message_ids = receipt_data[:MessageIDs]
    return if message_ids.blank?

    Rails.logger.info "ultralution Go API: Processing receipt for #{message_ids.size} message(s)"

    # Map ultralution Go receipt state to ultralution status
  ...
  def process_bulk_receipts(message_ids, status, _receipt_data)
    ...
    Rails.logger.info "ultralution Go API: Processing bulk receipt for #{message_ids.size} messages"
    ...
    if missing_ids.any?
      Rails.logger.warn "ultralution Go API: Missing messages for receipt: #{missing_ids.first(5).join(', ')}#{missing_ids.size > 5 ? " and #{missing_ids.size - 5} more" : ''}"
    end
    ...
    if updatable_messages.any?
      ...
      updatable_messages.each do |message|
        Rails.logger.debug { "ultralution Go API: Bulk updated message #{message.source_id} to #{status}" }
      end
    end
  ...
  def update_contact_activity_for_bulk_receipts(messages, status, receipt_data)
    ...
    contacts.update_all(last_activity_at: Time.current)
    Rails.logger.info "ultralution Go API: Updated last activity for #{contacts.count} contacts from bulk receipt"
  end
  ...
  def process_single_receipt(message_id, status, _receipt_data)
    ...
    Rails.logger.debug { "ultralution Go API: Updated message #{message.source_id} to #{status}" }
  end
  ...
  def find_message_by_source_id_for_receipt(message_id, _receipt_data)
    ...
    if message
      Rails.logger.debug do
        "ultralution Go API: Found message: #{message.id}, current status: #{message.status}, message_type: #{message.message_type}"
      end
    else
      Rails.logger.debug { "ultralution Go API: Message not found in inbox #{inbox.id} with source_id: #{message_id}" }
    end
  ...
  def update_message_status_from_receipt(message, status, message_id)
    ...
    unless valid_receipt_status_transition?(message.status, status)
      Rails.logger.warn "ultralution Go API: Invalid status transition for message #{message_id}: #{message.status} -> #{status}"
      return
    end

    # Update the message status
    if message.update(status: status)
      Rails.logger.info "ultralution Go API: Updated message #{message_id} status to #{status}"
    else
      Rails.logger.error "ultralution Go API: Failed to update message #{message_id}: #{message.errors.full_messages.join(', ')}"
    end
  end
  ...
  def update_contact_activity_if_needed(message, status, receipt_data)
    ...
    Rails.logger.info "ultralution Go API: Updated contact #{contact.id} last activity for read receipt"
  end
  ...
  def map_receipt_state_to_status(state, _type)
    ...
    when 'delivered'
      ...
      Rails.logger.warn "ultralution Go API: Unknown receipt state: #{state}"
      nil
    end
  end
