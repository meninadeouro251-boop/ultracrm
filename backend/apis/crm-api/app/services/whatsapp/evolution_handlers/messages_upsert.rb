...
module Whatsapp::ultralutionHandlers::MessagesUpsert
  include Whatsapp::ultralutionHandlers::Helpers
  include Whatsapp::ultralutionHandlers::AttachmentProcessor
  include Whatsapp::ultralutionHandlers::FileExtensions
  include Whatsapp::ultralutionHandlers::ContentHandlers
  include ultralutionHelper

  private

  def process_messages_upsert
    # Ultra API v2.3.1 sends single message data directly in 'data' field
    ...
    Rails.logger.info "Ultra API: Processing message #{raw_message_id} (fromMe: #{!incoming?})"

    if incoming?
      handle_message
    else
      # Handle outgoing messages with lock to avoid race conditions
      with_ultralution_channel_lock_on_outgoing_message(inbox.channel.id) { handle_message }
    end
  end
  ...
  def handle_message
    ...
    Rails.logger.info "Ultra API: Creating new message #{raw_message_id}"
    ...
    unless @contact
      ...
      Rails.logger.warn "Ultra API: Contact not found for message: #{raw_message_id}"
      return
    end
  ...
  def save_message_and_notify
    ...
    Rails.logger.info "Ultra API: Message created successfully - ID: #{@message.id}, Content: #{@message.content&.truncate(100)}"
  ...
  def update_conversation_status_if_needed
    ...
    if @conversation.inbox.active_bot?
      Rails.logger.info "Ultra API: Keeping conversation #{@conversation.id} pending (active bot present)"
      return
    end

    @conversation.update!(status: :open)
    Rails.logger.info "Ultra API: Updated conversation #{@conversation.id} status from pending to open for outgoing message"
  end
end
