module Whatsapp::ultralutionHandlers::AttachmentLogger
  def log_attachment_info(attachment_file, final_filename, final_content_type)
    Rails.logger.info 'Ultra API: Creating attachment with:'
  ...
  def log_attachment_success(attachment)
    Rails.logger.info "Ultra API: Successfully created attachment for message #{raw_message_id}"
    Rails.logger.info "Ultra API: Attachment ID: #{attachment.id}, File attached: #{attachment.file.attached?}"
  end

  def log_base64_processing(decoded_data, content_type, file_name)
    Rails.logger.info 'Ultra API: Creating attachment from base64'
  ...
  def log_tempfile_success(tempfile)
    Rails.logger.info "Ultra API: Successfully created tempfile: #{tempfile.path}"
    Rails.logger.info "Ultra API: Tempfile size: #{tempfile.size} bytes"
  end

  def log_base64_error(error, base64_data)
    Rails.logger.error "Ultra API: Failed to create file from base64: #{error.message}"
  ...
  def debug_media_info
    ...
    Rails.logger.info 'Ultra API: Media processing debug:'
