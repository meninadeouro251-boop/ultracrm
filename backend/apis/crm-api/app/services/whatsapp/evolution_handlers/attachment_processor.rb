...
module Whatsapp::ultralutionHandlers::AttachmentProcessor
  include Whatsapp::ultralutionHandlers::AttachmentLogger

  def handle_attach_media
    Rails.logger.info "Ultra API: Processing attachment for message #{raw_message_id}, type: #{message_type}"
    ...
    Rails.logger.error "Ultra API: Failed to download attachment for message #{raw_message_id}: #{e.message}"
  rescue StandardError => e
    Rails.logger.error "Ultra API: Failed to create attachment for message #{raw_message_id}: #{e.message}"
  ...
  def download_attachment_file
    ...
    if message[:base64].present?
      Rails.logger.info 'Ultra API: Processing base64 attachment'
      return create_tempfile_from_base64(message[:base64])
    end

    if message[:mediaUrl].present?
      Rails.logger.info "Ultra API: Downloading from mediaUrl: #{message[:mediaUrl]}"
      return Down.download(message[:mediaUrl], headers: inbox.channel.api_headers)
    end

    Rails.logger.warn 'Ultra API: No media found - no base64 or mediaUrl'
    nil
  rescue StandardError => e
    Rails.logger.error "Ultra API: Failed to download media: #{e.message}"
    nil
  end
