class Crm::Bms::Mappers::ContactMapper
  ...
  def map_custom_attributes
    ...
    # Map ultralution custom attributes to BMS custom fields
    contact.custom_attributes.each do |key, value|
      # Convert ultralution attribute keys to BMS custom field format
  ...
  def map_attribute_key(ultralution_key)
    # Map common ultralution attribute keys to BMS custom field keys
    key_mapping = {
      'cpf' => 'cpf',
      'gender' => 'gender',
    ...
    key_mapping[ultralution_key] || ultralution_key
  end
  ...
  def generate_device_token
    ...
    "ultralution-#{contact.id}-#{SecureRandom.hex(8)}"
  end
  ...
  def extract_browser_name(browser_info)
    return 'Ultra CRM' unless browser_info.is_a?(Hash)
