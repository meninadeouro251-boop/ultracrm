class Crm::Bms::ProcessorService < Crm::BaseProcessorService
  ...
  def create_or_update_contact(contact, contact_id)
    ...
    if contact_id.present?
      ...
      Rails.logger.info("Updated BMS contact #{contact_id} for ultralution contact #{contact.id}")
    else
      new_contact_id = @contact_finder.find_or_create(contact)
      store_external_id(contact, new_contact_id)
      Rails.logger.info("Created/Found BMS contact #{new_contact_id} for ultralution contact #{contact.id}")
    end
  rescue Crm::Bms::Api::BaseClient::ApiError => e
    ultralutionExceptionTracker.new(e).capture_exception
  ...
  def sync_label_to_bms(label)
    ...
    if stored_tag_id.present?
      ...
      Rails.logger.info("Updated BMS tag #{stored_tag_id} for ultralution label #{label.id}")
    else
      new_tag_id = @tag_client.create_tag(tag_data)
      store_label_external_id(label, new_tag_id)
      Rails.logger.info("Created BMS tag #{new_tag_id} for ultralution label #{label.id}")
    end
  rescue Crm::Bms::Api::BaseClient::ApiError => e
    ultralutionExceptionTracker.new(e).capture_exception
  ...
  def sync_custom_field_to_bms(custom_attribute_definition)
    ...
    if stored_field_id.present?
      ...
      Rails.logger.info("Updated BMS custom field #{stored_field_id} for ultralution custom attribute #{custom_attribute_definition.id}")
    else
      ...
      Rails.logger.info("Created BMS custom field #{new_field_id} for ultralution custom attribute #{custom_attribute_definition.id}")
    end
  rescue Crm::Bms::Api::BaseClient::ApiError => e
    ultralutionExceptionTracker.new(e).capture_exception
