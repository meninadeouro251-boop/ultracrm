...
class Facebook::Moderation::ResponseGeneratorService
  ...
  def call_agent_bot(payload)
    ...
    when 'ultra_ai_provider'
      call_ultra_ai_bot(payload)
    else
  ...
  def call_ultra_ai_bot(payload)
    # ultra AI provider uses JSON-RPC format (same as HttpRequestService)
    return nil unless agent_bot.outgoing_url.present?
    ...
    jsonrpc_payload = {
      ...
      params: {
        ...
        metadata: {
          ultra_crm_event: 'message_created',
          ultra_crm_data: payload,
  ...
  def call_http_bot(payload)
    # Generic HTTP bot call - use ultra_ai format
    call_ultra_ai_bot(payload)
  end
  ...
  def extract_response_content(parsed_response)
    ...
    # If content is in artifacts format (like ultra-ai)
