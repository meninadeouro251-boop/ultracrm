...
class UltraAuthService
  ...
  def initialize(base_url = nil)
    @base_url = base_url || ENV.fetch('ULTRA_AUTH_SERVICE_URL', 'http://localhost:3001')
  end

  # Validate existing token - Primary method for ultralution authentication
  def validate_token(token:, token_type:)
    ...
    Rails.logger.error "ultraAuth: Network error during validation: #{e.message}"
    raise AuthenticationError, 'Authentication service unavailable'
  rescue StandardError => e
    Rails.logger.error "ultraAuth: Token validation error: #{e.class} - #{e.message}"
  ...
  def get_user(user_id)
    ...
    Rails.logger.error "ultraAuth: Get user error: #{e.message}"
    nil
  end
  ...
  def get_role(user_id, _identifier = nil)
    ...
    Rails.logger.error "ultraAuth: Get role error: #{e.message}"
    nil
  end
  ...
  def perform_request(uri, request)
    ...
    Rails.logger.error "ultraAuth: HTTP error #{response.code}: #{response.message}"
    raise NetworkError, "HTTP #{response.code}: #{response.message}"
  rescue ValidationError => e
    raise e
  rescue ParseError => e
    Rails.logger.error "ultraAuth: Response parse error: #{e.message}"
    raise NetworkError, "Invalid response format"
  rescue Timeout::Error, Net::ReadTimeout, Net::OpenTimeout => e
    Rails.logger.error "ultraAuth: Request timeout: #{e.class} - #{e.message}"
    raise NetworkError, "Request timeout: #{e.message}"
  rescue Errno::ECONNREFUSED, Errno::EHOSTUNREACH, Errno::ETIMEDOUT => e
    Rails.logger.error "ultraAuth: Connection error: #{e.class} - #{e.message}"
    raise NetworkError, "Connection failed: #{e.message}"
  rescue SocketError => e
    Rails.logger.error "ultraAuth: Socket error: #{e.class} - #{e.message}"
    raise NetworkError, "Socket error: #{e.message}"
  rescue OpenSSL::SSL::SSLError => e
    Rails.logger.error "ultraAuth: SSL error: #{e.class} - #{e.message}"
    raise NetworkError, "SSL error: #{e.message}"
  rescue NetworkError => e
    raise e
  rescue StandardError => e
    Rails.logger.error "ultraAuth: Unexpected error: #{e.class} - #{e.message}"
  ...
  def persistent_http_client
    self.class.clients_mutex.synchronize do
      self.class.persistent_clients[@base_url] ||= begin
        client = Net::HTTP::Persistent.new(name: "ultra-auth-#{Digest::SHA1.hexdigest(@base_url)}")
  ...
  def instrument_remote_call(operation, metadata = {})
    ...
    Rails.logger.info("ultraAuthPerformance operation=#{operation} status=ok duration_ms=#{elapsed_ms} #{metadata.map { |k, v| "#{k}=#{v}" }.join(' ')}".strip)
    result
  rescue StandardError => e
    elapsed_ms = ((Process.clock_gettime(Process::CLOCK_MONOTONIC) - started_at) * 1000).round(1)
    Rails.logger.warn("ultraAuthPerformance operation=#{operation} status=error duration_ms=#{elapsed_ms} error=#{e.class} #{metadata.map { |k, v| "#{k}=#{v}" }.join(' ')}".strip)
    raise e
  end
