...
# Concern para verificacao de permissoes usando ultra-auth-service
# Este concern e usado nos controllers APOS a autenticacao
module ultraPermissionConcern
  ...
  def check_permission!(permission_key, type)
    ...
    if Current.service_authenticated == true
      Rails.logger.info "ultraPermission: Service token authenticated - granting access to #{permission_key}"
      return
    end
    ...
    unless user_id
      Rails.logger.error "ultraPermission: Missing user_id"
    ...
    unless has_permission
      Rails.logger.warn "ultraPermission: Access denied - user #{user_id} lacks #{permission_key}"
      render_permission_denied
      return
    end
  end
  ...
  def has_user_permission?(user_id, permission)
    Current.ultra_permission_cache ||= {}
    cache_key = "user:#{user_id}:#{permission}"
    return Current.ultra_permission_cache[cache_key] if Current.ultra_permission_cache.key?(cache_key)

    ultra_auth_service = UltraAuthService.new
    has_perm = ultra_auth_service.check_user_permission(user_id, permission)
    Current.ultra_permission_cache[cache_key] = has_perm
