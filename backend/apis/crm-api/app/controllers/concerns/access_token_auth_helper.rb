module AccessTokenAuthHelper
  ...
  def authenticate_access_token!
    ...
    Current.authentication_method = 'access_token'

    # Store the access token for use with other services (like ultra-campaign)
    token = request.headers[:api_access_token] || request.headers[:HTTP_API_ACCESS_TOKEN]
    Current.api_access_token = token if token.present?
  end
