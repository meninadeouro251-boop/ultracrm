...
module ultraAuthConcern
  ...
  def authenticate_user_with_ultra_auth(token, token_type)
    Current.ultra_auth_validation_cache ||= {}
    cache_key = ultra_auth_validation_cache_key(token, token_type)
    user_data = Current.ultra_auth_validation_cache[cache_key]

    auth_service = UltraAuthService.new
    unless user_data
      store_key = ultra_auth_validation_store_key(cache_key)
    ...
    Current.ultra_auth_validation_cache[cache_key] = user_data

    set_current_user_from_auth_data(user_data, token, token_type)
    true
  rescue UltraAuthService::ValidationError => e
    render_error('VALIDATION_ERROR', e.message, status: :unprocessable_entity)
  rescue UltraAuthService::AuthenticationError => e
  ...
  def set_current_user_from_auth_data(user_data, token, token_type)
    ...
    raise UltraAuthService::ValidationError, 'User not found locally' unless user
    ...
    # Store role key from ultra-auth for permission checks
    role_key = user_data.dig('user', 'role', 'key') || user_data.dig('role', 'key')
    Current.ultra_role_key = role_key
  ...
  def ultra_auth_validation_cache_key(token, token_type)
    "#{token_type}:#{Digest::SHA256.hexdigest(token.to_s)}"
  end

  def ultra_auth_validation_store_key(cache_key)
    "ultra_auth:validate:#{cache_key}"
  end
