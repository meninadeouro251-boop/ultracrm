...
class HealthController < ActionController::Base
  ...
  def update_concurrent_users_metric
    ...
    ULTRA_AI_CRM_CONCURRENT_USERS_GAUGE.set(value.to_i)
  ...
  def messages_total_metrics
    ...
    <<~METRICS
      # HELP ultra_ai_crm_messages_total Total CRM messages by direction
      # TYPE ultra_ai_crm_messages_total counter
      ultra_ai_crm_messages_total{direction="inbound"} #{inbound}
      ultra_ai_crm_messages_total{direction="outbound"} #{outbound}
    METRICS
  rescue StandardError => e
