class Api::V1::ultralutionGo::PrivacyController < Api::V1::BaseController
  before_action :set_instance_params, only: [:show, :update]

  # GET PRIVACY - GET /user/privacy
  def show
    Rails.logger.info "ultralution Go API: Getting privacy settings for instance #{@instance_uuid}"
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: Get privacy error: #{e.message}"
      render json: {
        error: e.message
  ...
  def update
    Rails.logger.info "ultralution Go API: Updating privacy settings for instance #{@instance_uuid}"
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: Update privacy error: #{e.message}"
      render json: {
        error: e.message
      }, status: :unprocessable_entity
    end
  ...
  def get_privacy_settings(api_url, instance_token)
    ...
    Rails.logger.info "ultralution Go API: Getting privacy settings at #{privacy_url}"
    ...
    Rails.logger.info "ultralution Go API: Get privacy response code: #{response.code}"
    Rails.logger.info "ultralution Go API: Get privacy response body: #{response.body}"

    raise "Failed to get privacy settings. Status: #{response.code}, Body: #{response.body}" unless response.is_a?(Net::HTTPSuccess)

    # ultralution Go retorna direto o objeto de privacy, não wrapped em 'data'
    JSON.parse(response.body)
  rescue JSON::ParserError => e
    Rails.logger.error "ultralution Go API: Get privacy JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from ultralution Go API get privacy endpoint'
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: Get privacy connection error: #{e.class} - #{e.message}"
    raise "Failed to get privacy settings: #{e.message}"
  end

  def update_privacy_settings(api_url, instance_token, privacy)
    privacy_url = "#{api_url.chomp('/')}/user/privacy"
    Rails.logger.info "ultralution Go API: Updating privacy settings at #{privacy_url}"

    # Configurações de privacidade do ultralution Go
    request_body = {
    ...
    Rails.logger.info "ultralution Go API: Update privacy request body: #{request.body}"

    response = http.request(request)
    Rails.logger.info "ultralution Go API: Update privacy response code: #{response.code}"
    Rails.logger.info "ultralution Go API: Update privacy response body: #{response.body}"
    ...
    Rails.logger.error "ultralution Go API: Update privacy JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from ultralution Go API update privacy endpoint'
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: Update privacy connection error: #{e.class} - #{e.message}"
