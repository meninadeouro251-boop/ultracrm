class Api::V1::ultralutionGo::ProfileController < Api::V1::BaseController
  ...
  def info
    Rails.logger.info "ultralution Go API: Getting user info for instance #{@instance_uuid}"
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: Get user info error: #{e.message}"
      render json: {
        error: e.message
  ...
  def avatar
    Rails.logger.info "ultralution Go API: Getting avatar for instance #{@instance_uuid}"
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: Get avatar error: #{e.message}"
      render json: {
        error: e.message
      }, status: :unprocessable_entity
    end
  ...
  def update_picture
    Rails.logger.info "ultralution Go API: Updating profile picture for instance #{@instance_uuid}"
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: Update profile picture error: #{e.message}"
      render json: {
        error: e.message
      }, status: :unprocessable_entity
    end
  ...
  def show
    Rails.logger.info "ultralution Go API: Getting profile for instance #{@instance_uuid}"
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: Get profile error: #{e.message}"
  ...
  def update_name
    Rails.logger.info "ultralution Go API: Updating profile name for instance #{@instance_uuid}"
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: Update profile name error: #{e.message}"
      render json: { error: e.message }, status: :unprocessable_entity
    end
  end
  ...
  def update_status
    Rails.logger.info "ultralution Go API: Updating profile status for instance #{@instance_uuid}"
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: Update profile status error: #{e.message}"
      render json: { error: e.message }, status: :unprocessable_entity
    end
  end
  ...
  def remove_picture
    Rails.logger.info "ultralution Go API: Removing profile picture for instance #{@instance_uuid}"
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: Remove profile picture error: #{e.message}"
      render json: { error: e.message }, status: :unprocessable_entity
    end
  end
  ...
  def get_user_info(api_url, instance_token, phone_numbers)
    ...
    Rails.logger.info "ultralution Go API: Getting user info at #{info_url} for numbers: #{phone_numbers.join(', ')}"
    ...
    Rails.logger.info "ultralution Go API: User info request body: #{request.body}"

    response = http.request(request)
    Rails.logger.info "ultralution Go API: User info response code: #{response.code}"
    Rails.logger.info "ultralution Go API: User info response body: #{response.body}"

    raise "Failed to get user info. Status: #{response.code}, Body: #{response.body}" unless response.is_a?(Net::HTTPSuccess)

    JSON.parse(response.body)
  rescue JSON::ParserError => e
    Rails.logger.error "ultralution Go API: User info JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from ultralution Go API user info endpoint'
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: User info connection error: #{e.class} - #{e.message}"
  ...
  def get_avatar(api_url, instance_token, phone_number, preview)
    ...
    Rails.logger.info "ultralution Go API: Getting avatar at #{avatar_url} for number: #{phone_number}"
    ...
    Rails.logger.info "ultralution Go API: Avatar request body: #{request.body}"

    response = http.request(request)
    Rails.logger.info "ultralution Go API: Avatar response code: #{response.code}"
    Rails.logger.info "ultralution Go API: Avatar response body: #{response.body[0..200]}" # First 200 chars

    raise "Failed to get avatar. Status: #{response.code}, Body: #{response.body}" unless response.is_a?(Net::HTTPSuccess)

    JSON.parse(response.body)
  rescue JSON::ParserError => e
    Rails.logger.error "ultralution Go API: Avatar JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from ultralution Go API avatar endpoint'
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: Avatar connection error: #{e.class} - #{e.message}"
  ...
  def make_post_request(url, instance_token, body)
    Rails.logger.info "ultralution Go API: POST #{url}"
    uri = URI.parse(url)
    http = Net::HTTP.new(uri.host, uri.port)
    http.use_ssl = (uri.scheme == 'https')
    http.open_timeout = 15
    ...
    response = http.request(request)
    Rails.logger.info "ultralution Go API: Response code: #{response.code}"
  ...
  def update_profile_picture(api_url, instance_token, image_url)
    ...
    Rails.logger.info "ultralution Go API: Updating profile picture at #{picture_url}"
    ...
    Rails.logger.info "ultralution Go API: Update picture request body: #{request.body}"

    response = http.request(request)
    Rails.logger.info "ultralution Go API: Update picture response code: #{response.code}"
    Rails.logger.info "ultralution Go API: Update picture response body: #{response.body}"

    raise "Failed to update profile picture. Status: #{response.code}, Body: #{response.body}" unless response.is_a?(Net::HTTPSuccess)

    JSON.parse(response.body)
  rescue JSON::ParserError => e
    Rails.logger.error "ultralution Go API: Update picture JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from ultralution Go API update picture endpoint'
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: Update picture connection error: #{e.class} - #{e.message}"
