class Api::V1::ultralutionGo::QrcodesController < Api::V1::BaseController
  include ultralutionGoConcern

  before_action :set_instance_params, only: [:show, :create]

  def show
    Rails.logger.info "ultralution Go API: Getting QR code for instance #{@instance_uuid}"
    ...
    begin
      ...
      # Get QR code using ultralution Go API
      qrcode_data = get_qrcode_go(@api_url, @instance_token)

      render json: qrcode_data
    rescue StandardError => e
      Rails.logger.error "ultralution Go API: QR code error: #{e.class} - #{e.message}"
  ...
  def create
    Rails.logger.info "ultralution Go API: QR code refresh called with params: #{params.inspect}"
  ...
  def set_instance_params
    ...
    Rails.logger.info "ultralution Go API: Looking for instance with identifier: #{identifier}"
  ...
  def get_qrcode_go(api_url, instance_token)
    # ultralution Go API endpoint: GET /instance/qr
    qrcode_url = "#{api_url.chomp('/')}/instance/qr"
    Rails.logger.info "ultralution Go API: Getting QR code from #{qrcode_url}"
