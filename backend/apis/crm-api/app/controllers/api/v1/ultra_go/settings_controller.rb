class Api::V1::UltraGo::SettingsController < Api::V1::BaseController
  before_action :set_instance_params, only: [:show, :update]

  # GET SETTINGS - GET /instance/:instanceId/advanced-settings
  def show
    Rails.logger.info "ultralution Go API: Getting advanced settings for instance #{@instance_uuid}"
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: Get settings error: #{e.message}"
      render json: {
        error: e.message
  ...
  def update
    Rails.logger.info "ultralution Go API: Updating advanced settings for instance #{@instance_uuid}"
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: Update settings error: #{e.message}"
      render json: {
        error: e.message
      }, status: :unprocessable_entity
    end
  ...
  def get_advanced_settings(api_url, instance_token, instance_uuid)
    ...
    Rails.logger.info "ultralution Go API: Getting advanced settings at #{settings_url}"
    Rails.logger.info "ultralution Go API: Using instance_token: #{instance_token ? '[PRESENT]' : '[MISSING]'}"
    Rails.logger.info "ultralution Go API: Token length: #{instance_token&.length || 0}"
    ...
    Rails.logger.info "ultralution Go API: Get settings response code: #{response.code}"
    Rails.logger.info "ultralution Go API: Get settings response body: #{response.body}"

    raise "Failed to get advanced settings. Status: #{response.code}, Body: #{response.body}" unless response.is_a?(Net::HTTPSuccess)

    # ultralution Go retorna direto o objeto de settings, não wrapped em 'data'
    JSON.parse(response.body)
  rescue JSON::ParserError => e
    Rails.logger.error "ultralution Go API: Get settings JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from ultralution Go API get settings endpoint'
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: Get settings connection error: #{e.class} - #{e.message}"
  ...
  def update_advanced_settings(api_url, instance_token, instance_uuid, settings)
    settings_url = "#{api_url.chomp('/')}/instance/#{instance_uuid}/advanced-settings"
    Rails.logger.info "ultralution Go API: Updating advanced settings at #{settings_url}"

    # Configurações padrão do ultralution Go
    request_body = {
    ...
    Rails.logger.info "ultralution Go API: Update settings request body: #{request.body}"

    response = http.request(request)
    Rails.logger.info "ultralution Go API: Update settings response code: #{response.code}"
    Rails.logger.info "ultralution Go API: Update settings response body: #{response.body}"

    raise "Failed to update advanced settings. Status: #{response.code}, Body: #{response.body}" unless response.is_a?(Net::HTTPSuccess)

    # ultralution Go retorna direto o objeto, não wrapped em 'data'
    JSON.parse(response.body)
  rescue JSON::ParserError => e
    Rails.logger.error "ultralution Go API: Update settings JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from ultralution Go API update settings endpoint'
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: Update settings connection error: #{e.class} - #{e.message}"
