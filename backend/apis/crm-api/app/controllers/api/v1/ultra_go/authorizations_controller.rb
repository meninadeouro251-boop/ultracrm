class Api::V1::UltraGo::AuthorizationsController < Api::V1::BaseController
  include UltraGoConcern
  ...
  def create
    Rails.logger.info "ultralution Go API: Creating instance with params: #{params.inspect}"
    ...
    if missing_params.any?
      Rails.logger.warn "ultralution Go API: Missing parameters: #{missing_params.join(', ')}. Params: #{params.inspect}"
      return render json: {
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: Instance creation error: #{e.message}"
      render json: {
        error: e.message
  ...
  def connect
    Rails.logger.info "ultralution Go API: Connecting instance #{@instance_name}"
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: Instance connection error: #{e.message}"
      render json: {
        error: e.message
      }, status: :unprocessable_entity
    end
  ...
  def qrcode
    Rails.logger.info "ultralution Go API: Getting QR code for instance #{@instance_name}"
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: QR code error: #{e.message}"
      render json: {
        error: e.message
      }, status: :unprocessable_entity
    end
  ...
  def fetch
    Rails.logger.info "ultralution Go API: Fetching instance info for #{@instance_uuid}"
    ...
    begin
      ...
      if @inbox
        ...
        if is_connected
          Rails.logger.info "ultralution Go API: Instance #{@instance_uuid} is connected, updating channel status"
          ...
          if instance_data['jid'].present?
            Rails.logger.info "ultralution Go API: Instance connected, fetching profile picture for #{instance_data['jid']}"
            begin
              ...
              Rails.logger.error "ultralution Go API: Failed to update profile picture: #{e.message}"
              # Don't raise - profile picture update is not critical
            end
          end
        else
          Rails.logger.info "ultralution Go API: Instance #{@instance_uuid} is disconnected, updating channel status"
      ...
      # Convert ultralution Go format to frontend-expected format
      ...
      Rails.logger.error "ultralution Go API: Fetch instance error: #{e.message}"
      render json: {
        error: e.message
      }, status: :unprocessable_entity
    end
  ...
  def logout
    Rails.logger.info "ultralution Go API: Logging out instance #{@instance_name}"
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: Logout error: #{e.message}"
      render json: {
        error: e.message
      }, status: :unprocessable_entity
    end
  ...
  def delete_instance
    Rails.logger.info "ultralution Go API: Deleting instance #{@instance_uuid}"
    ...
    begin
      ...
      Rails.logger.error "ultralution Go API: Delete instance error: #{e.message}"
      render json: {
        error: e.message
      }, status: :unprocessable_entity
    end
  ...
  def set_instance_params
    ...
    @api_url = auth_params[:api_url].presence || GlobalConfigService.load('ULTRA_GO_API_URL', '').to_s.strip
    @admin_token = auth_params[:admin_token].presence || GlobalConfigService.load('ULTRA_GO_ADMIN_SECRET', '').to_s.strip
    ...
    Rails.logger.info "ultralution Go API: Looking for instance with identifier: #{@instance_uuid}"

    # Try to find the channel first by instance UUID
    whatsapp_channel = Channel::Whatsapp.joins(:inbox)
                                        .where(provider: 'ultralution_go')
                                        .find { |ch| ch.provider_config['instance_uuid'] == @instance_uuid }

    if whatsapp_channel
      Rails.logger.info "ultralution Go API: Found channel with config: #{whatsapp_channel.provider_config.inspect}"
      ...
      Rails.logger.warn "ultralution Go API: No channel found for instance_uuid: #{@instance_uuid}"
    end
  end

  def create_instance_go(api_url, admin_token, instance_name, auth_params)
    create_url = "#{api_url.chomp('/')}/instance/create"
    Rails.logger.info "ultralution Go API: Creating instance at #{create_url}"
    ...
    Rails.logger.info "ultralution Go API: Using instance settings: #{instance_settings.inspect}"
    ...
    Rails.logger.info "ultralution Go API: Create instance request body: #{request.body}"

    response = http.request(request)
    Rails.logger.info "ultralution Go API: Create instance response code: #{response.code}"
    Rails.logger.info "ultralution Go API: Create instance response body: #{response.body}"

    raise "Failed to create instance. Status: #{response.code}, Body: #{response.body}" unless response.is_a?(Net::HTTPSuccess)

    response_data = JSON.parse(response.body)

    # Usar o token retornado pela API ultralution Go (não o gerado localmente)
    api_returned_token = response_data.dig('data', 'token') || response_data['token']

    Rails.logger.info "ultralution Go API: Generated token: #{instance_token}"
    Rails.logger.info "ultralution Go API: API returned token: #{api_returned_token ? '[PRESENT]' : '[MISSING]'}"

    # Se a API não retornou token, usar o que geramos
    final_token = api_returned_token || instance_token
    Rails.logger.info "ultralution Go API: Final token to save: #{final_token ? '[PRESENT]' : '[MISSING]'}"
    ...
    Rails.logger.error "ultralution Go API: Create instance JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from ultralution Go API create instance endpoint'
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: Create instance connection error: #{e.class} - #{e.message}"
    raise "Failed to create instance: #{e.message}"
  end

  def check_server_status_go(api_url)
    status_url = "#{api_url.chomp('/')}/server/ok"
    Rails.logger.info "ultralution Go API: Checking server status at #{status_url}"
    ...
    response = http.request(request)
    Rails.logger.info "ultralution Go API: Server status response code: #{response.code}"
    Rails.logger.info "ultralution Go API: Server status response body: #{response.body}"
    ...
    Rails.logger.error "ultralution Go API: Server status JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from ultralution Go API server status endpoint'
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: Server status connection error: #{e.class} - #{e.message}"
    raise "Failed to verify connection: #{e.message}"
  end

  def get_qrcode_go(api_url, instance_token)
    qrcode_url = "#{api_url.chomp('/')}/instance/qr"
    Rails.logger.info "ultralution Go API: Getting QR code at #{qrcode_url}"
    ...
    response = http.request(request)
    Rails.logger.info "ultralution Go API: QR code response code: #{response.code}"
    Rails.logger.info "ultralution Go API: QR code response body: #{response.body}"

    raise "Failed to get QR code. Status: #{response.code}, Body: #{response.body}" unless response.is_a?(Net::HTTPSuccess)

    JSON.parse(response.body)
  rescue JSON::ParserError => e
    Rails.logger.error "ultralution Go API: QR code JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from ultralution Go API QR code endpoint'
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: QR code connection error: #{e.class} - #{e.message}"
  ...
  def fetch_instance_info(api_url, admin_token, instance_uuid)
    ...
    Rails.logger.info "ultralution Go API: Fetching instance info at #{fetch_url}"
    ...
    response = http.request(request)
    Rails.logger.info "ultralution Go API: Fetch instance response code: #{response.code}"
    Rails.logger.info "ultralution Go API: Fetch instance response body: #{response.body}"

    raise "Failed to fetch instance info. Status: #{response.code}, Body: #{response.body}" unless response.is_a?(Net::HTTPSuccess)

    response_data = JSON.parse(response.body)

    # A API ultralution Go retorna: { "data": {...}, "message": "success" }
    # Extrair o campo 'data' que contém as informações da instância
    response_data['data'] || response_data
  rescue JSON::ParserError => e
    Rails.logger.error "ultralution Go API: Fetch instance JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from ultralution Go API fetch instance endpoint'
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: Fetch instance connection error: #{e.class} - #{e.message}"
  ...
  def logout_instance(api_url, instance_token)
    ...
    Rails.logger.info "ultralution Go API: Logging out instance at #{logout_url}"
    ...
    response = http.request(request)
    Rails.logger.info "ultralution Go API: Logout response code: #{response.code}"
    Rails.logger.info "ultralution Go API: Logout response body: #{response.body}"

    raise "Failed to logout instance. Status: #{response.code}, Body: #{response.body}" unless response.is_a?(Net::HTTPSuccess)

    JSON.parse(response.body) if response.body.present?
  rescue JSON::ParserError => e
    Rails.logger.error "ultralution Go API: Logout JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from ultralution Go API logout endpoint'
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: Logout connection error: #{e.class} - #{e.message}"
  ...
  def delete_instance_go(api_url, instance_token, instance_uuid)
    ...
    Rails.logger.info "ultralution Go API: Deleting instance at #{delete_url}"
    ...
    response = http.request(request)
    Rails.logger.info "ultralution Go API: Delete instance response code: #{response.code}"
    Rails.logger.info "ultralution Go API: Delete instance response body: #{response.body}"

    raise "Failed to delete instance. Status: #{response.code}, Body: #{response.body}" unless response.is_a?(Net::HTTPSuccess)

    JSON.parse(response.body) if response.body.present?
  rescue JSON::ParserError => e
    Rails.logger.error "ultralution Go API: Delete instance JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from ultralution Go API delete endpoint'
  rescue StandardError => e
    Rails.logger.error "ultralution Go API: Delete instance connection error: #{e.class} - #{e.message}"
    raise "Failed to delete instance: #{e.message}"
  end

  def check_and_delete_existing_instance(_api_url, _admin_token, instance_name)
    Rails.logger.info "ultralution Go API: Checking if instance #{instance_name} already exists"
    Rails.logger.info 'ultralution Go API: Currently using instance management approach for ultralution Go'
  rescue StandardError => e
    Rails.logger.info "ultralution Go API: Instance #{instance_name} doesn't exist (#{e.message}), proceeding with creation"
  end
  ...
  def fetch_and_update_inbox_avatar(phone_number, api_url, instance_token)
    ...
    if avatar_url.present?
      ...
      Rails.logger.info "ultralution Go API: Successfully updated inbox #{@inbox.id} avatar via fetch endpoint"
    else
