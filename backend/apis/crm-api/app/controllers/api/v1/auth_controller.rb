class Api::V1::AuthController < ApplicationController
  include ultraAuthConcern

  skip_before_action :verify_authenticity_token, raise: false
  skip_before_action :authenticate_with_ultra_auth!, except: [:me, :logout], raise: false
  ...
  def login
    ...
    if result[:success]
      ...
      session[:ultra_auth_token] = result[:token]
  ...
  def refresh
    refresh_token = params[:refresh_token] || session[:ultra_refresh_token]
    ...
    if result[:success]
      session[:ultra_auth_token] = result[:token]
      session[:ultra_refresh_token] = result[:refresh_token]
