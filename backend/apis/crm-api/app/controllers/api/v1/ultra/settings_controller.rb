class Api::V1::ultralution::SettingsController < Api::V1::BaseController
  def show
    Rails.logger.info "Ultra API get settings called for instance: #{params[:id]}"
    ...
    begin
      ...
      Rails.logger.error "Ultra API get settings error: #{e.class} - #{e.message}"
      render json: { error: e.message }, status: :unprocessable_entity
    end
  end

  def create
    Rails.logger.info "Ultra API settings configuration called with params: #{params.inspect}"
    ...
    begin
      ...
      Rails.logger.info "Ultra API: Setting instance settings for #{instance_name}"
      ...
      Rails.logger.error "Ultra API settings configuration error: #{e.class} - #{e.message}"
  ...
  def update
    Rails.logger.info "Ultra API update settings called for instance: #{params[:id]}"
    ...
    begin
      ...
      Rails.logger.error "Ultra API update settings error: #{e.class} - #{e.message}"
      render json: { error: e.message }, status: :unprocessable_entity
    end
  end
  ...
  def get_settings(api_url, api_hash, instance_name)
    ...
    Rails.logger.info "Ultra API: Getting instance settings from #{settings_url}"
    ...
    Rails.logger.info "Ultra API: Get settings response code: #{response.code}"
    Rails.logger.info "Ultra API: Get settings response body: #{response.body}"
    ...
    Rails.logger.error "Ultra API: Get settings JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from Ultra API get settings endpoint'
  rescue StandardError => e
    Rails.logger.error "Ultra API: Get settings connection error: #{e.class} - #{e.message}"
  ...
  def set_settings(api_url, api_hash, instance_name, instance_settings)
    ...
    Rails.logger.info "Ultra API: Setting instance settings at #{settings_url}"
    ...
    Rails.logger.info "Ultra API: Settings request headers: #{request.to_hash}"
    Rails.logger.info "Ultra API: Settings request body: #{request.body}"

    response = http.request(request)
    Rails.logger.info "Ultra API: Settings response code: #{response.code}"
    Rails.logger.info "Ultra API: Settings response body: #{response.body}"

    raise "Failed to set instance settings. Status: #{response.code}, Body: #{response.body}" unless response.is_a?(Net::HTTPSuccess)

    JSON.parse(response.body)
  rescue JSON::ParserError => e
    Rails.logger.error "Ultra API: Settings JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from Ultra API settings endpoint'
  rescue StandardError => e
    Rails.logger.error "Ultra API: Settings connection error: #{e.class} - #{e.message}"
