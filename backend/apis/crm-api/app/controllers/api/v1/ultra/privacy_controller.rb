class Api::V1::ultralution::PrivacyController < Api::V1::BaseController
  before_action :set_channel, only: [:show, :update]

  # GET /api/v1/ultralution/privacy/:id
  def show
    Rails.logger.info "Ultra API: Getting privacy settings for instance #{params[:id]}"
    ...
    begin
      ...
      Rails.logger.error "Ultra API: Get privacy error: #{e.class} - #{e.message}"
      render json: { error: e.message }, status: :unprocessable_entity
    end
  end

  # PUT /api/v1/ultralution/privacy/:id
  def update
    Rails.logger.info "Ultra API: Updating privacy settings for instance #{params[:id]}"
    ...
    begin
      ...
      Rails.logger.error "Ultra API: Update privacy error: #{e.class} - #{e.message}"
      render json: { error: e.message }, status: :unprocessable_entity
    end
  end
  ...
  def fetch_privacy_settings(api_url, api_hash, instance_name)
    ...
    Rails.logger.info "Ultra API: Fetching privacy from #{url}"
    ...
    Rails.logger.info "Ultra API: Get privacy response #{response.code}: #{response.body}"
    ...
    raise 'Invalid response from Ultra API privacy endpoint'
  rescue StandardError => e
  ...
  def update_privacy_settings(api_url, api_hash, instance_name, privacy)
    ...
    Rails.logger.info "Ultra API: Updating privacy at #{url}"
    ...
    Rails.logger.info "Ultra API: Update privacy body: #{request.body}"

    response = http.request(request)
    Rails.logger.info "Ultra API: Update privacy response #{response.code}: #{response.body}"

    unless response.is_a?(Net::HTTPSuccess)
      Rails.logger.error "Ultra API: Update privacy FAILED — Status: #{response.code}, Body: #{response.body}, Sent body: #{request_body.to_json}"
      raise "Failed to update privacy settings. Status: #{response.code}, Body: #{response.body}"
    end

    JSON.parse(response.body)
  rescue JSON::ParserError
    raise 'Invalid response from Ultra API update privacy endpoint'
  rescue StandardError => e
