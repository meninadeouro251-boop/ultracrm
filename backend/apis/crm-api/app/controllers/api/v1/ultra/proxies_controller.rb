class Api::V1::ultralution::ProxiesController < Api::V1::BaseController
  def show
    Rails.logger.info "Ultra API get proxy called for instance: #{params[:id]}"
    ...
    begin
      ...
      Rails.logger.error "Ultra API get proxy error: #{e.class} - #{e.message}"
      render json: { error: e.message }, status: :unprocessable_entity
    end
  end

  def create
    Rails.logger.info "Ultra API proxy configuration called with params: #{params.inspect}"
    ...
    begin
      ...
      Rails.logger.info "Ultra API: Setting proxy for instance #{instance_name}"
      ...
      Rails.logger.error "Ultra API proxy configuration error: #{e.class} - #{e.message}"
  ...
  def get_proxy(api_url, api_hash, instance_name)
    ...
    Rails.logger.info "Ultra API: Getting proxy settings from #{proxy_url}"
    ...
    Rails.logger.info "Ultra API: Get proxy response code: #{response.code}"
    Rails.logger.info "Ultra API: Get proxy response body: #{response.body}"
    ...
    Rails.logger.error "Ultra API: Get proxy JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from Ultra API get proxy endpoint'
  rescue StandardError => e
    Rails.logger.error "Ultra API: Get proxy connection error: #{e.class} - #{e.message}"
  ...
  def set_proxy(api_url, api_hash, instance_name, proxy_settings)
    ...
    Rails.logger.info "Ultra API: Setting proxy at #{proxy_url}"
    ...
    Rails.logger.info "Ultra API: Proxy request headers: #{request.to_hash}"
    Rails.logger.info "Ultra API: Proxy request body: #{request.body}"

    response = http.request(request)
    Rails.logger.info "Ultra API: Proxy response code: #{response.code}"
    Rails.logger.info "Ultra API: Proxy response body: #{response.body}"

    raise "Failed to set proxy. Status: #{response.code}, Body: #{response.body}" unless response.is_a?(Net::HTTPSuccess)

    JSON.parse(response.body)
  rescue JSON::ParserError => e
    Rails.logger.error "Ultra API: Proxy JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from Ultra API proxy endpoint'
  rescue StandardError => e
    Rails.logger.error "Ultra API: Proxy connection error: #{e.class} - #{e.message}"
