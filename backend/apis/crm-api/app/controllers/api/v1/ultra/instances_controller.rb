class Api::V1::ultralution::InstancesController < Api::V1::BaseController
  def index
    Rails.logger.info "Ultra API fetch instances called with params: #{params.inspect}"
    ...
    begin
      ...
      Rails.logger.error "Ultra API fetch instances error: #{e.class} - #{e.message}"
      render json: { error: e.message }, status: :unprocessable_entity
    end
  end

  def logout
    Rails.logger.info "Ultra API logout called for instance: #{params[:id]}"
    ...
    begin
      ...
      Rails.logger.error "Ultra API logout error: #{e.class} - #{e.message}"
      render json: { error: e.message }, status: :unprocessable_entity
    end
  end
  ...
  def get_instance_info(api_url, api_hash, instance_name)
    ...
    Rails.logger.info "Ultra API: Getting instance info from #{info_url}"
    ...
    Rails.logger.info "Ultra API: Get instance info response code: #{response.code}"
    Rails.logger.info "Ultra API: Get instance info response body: #{response.body}"
    ...
    Rails.logger.error "Ultra API: Get instance info JSON parse error: #{e.message}, Body: #{response&.body}"
    ...
    Rails.logger.error "Ultra API: Get instance info connection error: #{e.class} - #{e.message}"
  ...
  def logout_instance(api_url, api_hash, instance_name)
    ...
    Rails.logger.info "Ultra API: Logging out instance at #{logout_url}"
    ...
    response = http.request(request)
    Rails.logger.info "Ultra API: Logout response code: #{response.code}"
    Rails.logger.info "Ultra API: Logout response body: #{response.body}"
    ...
    if response.code == '400' || response.code == '500'
      ...
      if messages.include?('not connected') || messages.include?('Connection Closed')
        Rails.logger.info "Ultra API: Instance already disconnected (#{response.code}), treating as successful logout"
    ...
    Rails.logger.error "Ultra API: Logout JSON parse error: #{e.message}, Body: #{response&.body}"
    raise 'Invalid response from Ultra API logout endpoint'
  rescue StandardError => e
    Rails.logger.error "Ultra API: Logout connection error: #{e.class} - #{e.message}"
