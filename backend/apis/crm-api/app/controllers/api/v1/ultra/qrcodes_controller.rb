class Api::V1::ultralution::QrcodesController < Api::V1::BaseController
  def show
    Rails.logger.info "Ultra API get QR code called for instance: #{params[:id]}"
    ...
    begin
      ...
      Rails.logger.error "Ultra API get QR code error: #{e.class} - #{e.message}"
      render json: { error: e.message }, status: :unprocessable_entity
    end
  end

  def create
    Rails.logger.info "Ultra API QR code refresh called with params: #{params.inspect}"
    ...
    begin
      ...
      Rails.logger.info "Ultra API: Refreshing QR code for instance #{instance_name}"
      ...
      Rails.logger.error "Ultra API QR code refresh error: #{e.class} - #{e.message}"
  ...
  def get_qrcode(api_url, api_hash, instance_name)
    ...
    Rails.logger.info "Ultra API: Getting QR code from #{qrcode_url}"
    ...
    raise 'Invalid response from Ultra API connect endpoint'
  rescue StandardError => e
