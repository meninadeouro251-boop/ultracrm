class Webhooks::WhatsappController < ActionController::API
  ...
  def process_payload
    # Check if this is an ultralution Go webhook payload
    if ultralution_go_payload?
      Rails.logger.info 'ultralution Go webhook detected, processing with ultralution Go handler'
      return process_ultralution_go_payload
    end
  ...
  def process_ultralution_go_payload
    Rails.logger.info "ultralution Go webhook received: #{params.slice(:event, :instanceId, :instanceToken)}"

    # ultralution Go webhook structure validation
    unless valid_ultralution_go_payload?
      Rails.logger.warn 'Invalid ultralution Go webhook payload: missing required fields'
      render json: { error: 'Invalid ultralution Go webhook payload' }, status: :bad_request
      return
    end

    # Process ultralution Go webhook
    Webhooks::WhatsappEventsJob.perform_later(params.to_unsafe_hash.merge(ultralution_go: true))
    head :ok
  end

  private

  def valid_ultralution_go_payload?
    # ultralution Go webhook must have: event, data, instanceId, instanceToken
  ...
  def ultralution_go_payload?
    # ultralution Go webhooks have instanceId and instanceToken at root level
