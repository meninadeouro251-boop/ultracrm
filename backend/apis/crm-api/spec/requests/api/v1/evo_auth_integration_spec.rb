...
RSpec.describe 'ultraAuth integration through API auth filter', type: :request do
  ...
  around do |example|
    original_base_url = ENV['ULTRA_AUTH_SERVICE_URL']
    ENV['ULTRA_AUTH_SERVICE_URL'] = base_url
    Rails.cache.clear
    Current.reset
    example.run
    Rails.cache.clear
    Current.reset
    ENV['ULTRA_AUTH_SERVICE_URL'] = original_base_url
  end
