    ...
    context 'when authenticated as admin' do
      ...
      context 'with valid config type' do
        ...
        it 'clears Redis cache after saving' do
          ...
          post :create, params: {
            config_type: 'ultralution',
            app_config: {
              ultraLUTION_API_URL: 'https://api.example.com',
              ultraLUTION_ADMIN_SECRET: 'secret-xyz'
            }
          }, format: :json
      ...
      context 'with required keys enforcement' do
        ...
        integrations = {
          ...
          'ultralution' => {
            required: %w[ultraLUTION_API_URL ultraLUTION_ADMIN_SECRET],
            full: { ultraLUTION_API_URL: 'https://api.example.com', ultraLUTION_ADMIN_SECRET: 'ev-secret' }
          },
          'ultralution_go' => {
            required: %w[ultraLUTION_GO_API_URL ultraLUTION_GO_ADMIN_SECRET],
            full: { ultraLUTION_GO_API_URL: 'https://go.example.com', ultraLUTION_GO_ADMIN_SECRET: 'go-secret' }
          },
          'twitter' => {
    ...
    context 'when authenticated as admin' do
      ...
      context 'with unsupported config type for testing' do
        it 'returns unsupported message' do
          post :test_connection, params: { config_type: 'ultralution' }, format: :json
