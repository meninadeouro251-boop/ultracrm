...
if Rails.env.production?
  ...
  Redis::Alfred.set(Redis::Alfred::ultraLUTION_INSTALLATION_ONBOARDING, true)
end

## Seeds for Local Development
unless Rails.env.production?

  # Note: User management is now handled by ultra-auth-service
  # For development, users should be created through the auth service
  # and then synced to ultralution using the authentication endpoints

  # To create users for development:
  # 1. Use the ultra-auth-service API to create users
  # 2. Users will be automatically synced to ultralution when they authenticate

  # For this seed to work in development, make sure you have users
  # created in ultra-auth-service first, then uncomment and adjust the lines below:

  # user = User.find_by(email: 'your-user@example.com')
  # if user.nil?
  #   Rails.logger.warn "⚠️  No user found. Please create users via ultra-auth-service first."
  #   Rails.logger.info "💡 Run: rails db:seed in ultra-auth-service to create default admin user"
  #   exit 1
  # end


  user = User.find_by(email: "support@ultra-auth-service-community.com")

  if user.nil?
    Rails.logger.warn "⚠️  No admin user found with email 'support@ultra-auth-service-community.com'"
    Rails.logger.info "💡 Please create this user in ultra-auth-service-community first:"
    Rails.logger.info "   1. Start ultra-auth-service-community"
    Rails.logger.info "   2. Run: rails db:seed in ultra-auth-service-community"
  ...
  CannedResponse.find_or_create_by!(short_code: 'start') do |canned_response|
    canned_response.content = 'Olá, bem-vindo ao Ultra CRM.'
  end
end
