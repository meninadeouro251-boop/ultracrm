# Ultra API - AI Agent Guidelines

This document provides comprehensive guidelines for AI agents (Claude, GPT, Cursor, etc.) working with the Ultra API codebase.

## Project Overview

**Ultra API** is a production-ready, multi-tenant WhatsApp API platform built with Node.js, TypeScript, and Express.js. It supports multiple WhatsApp providers and extensive integrations with chatbots, CRM systems, and messaging platforms.
...
- **`src/`** – TypeScript source code with modular architecture
  ...
  - `api/integrations/` – External service integrations
    - `channel/` – WhatsApp providers (Baileys, Business API, ultralution)
...
export class ExampleService {
  ...
  public async find(instance: InstanceDto): Promise<ExampleDto | null> {
    try {
      ...
      return result || null; // Return null on not found (ultralution pattern)
    } catch (error) {
      this.logger.error('Error finding data:', error);
      return null; // Return null on error (ultralution pattern)
    }
  }
}
```
...
```typescript
// CORRECT - Ultra API pattern (no decorators)
...
export class BadExampleDto {
  @IsString() // ❌ Ultra API doesn't use decorators
  name: string;
}
```
...
- **Ultra API**: Custom WhatsApp integration
...
- **Available integrations**: ultralutionBot, OpenAI, Dify, Typebot, Chatwoot, Flowise, N8N, ultraAI
