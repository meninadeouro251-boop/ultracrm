...
import { useFindDefaultSettingsultralutionBot } from "@/lib/queries/ultralutionBot/findDefaultSettingsultralutionBot";
import { useFindultralutionBot } from "@/lib/queries/ultralutionBot/findultralutionBot";
import { useManageultralutionBot } from "@/lib/queries/ultralutionBot/manageultralutionBot";

import { ultralutionBotSettings } from "@/types/ultralution.types";
...
function DefaultSettingsultralutionBot() {
  const { t } = useTranslation();
  const { instance } = useInstance();

  const [open, setOpen] = useState(false);
  const { data: settings, refetch: refetchSettings } = useFindDefaultSettingsultralutionBot({
    instanceName: instance?.name,
    enabled: open,
  });

  const { data: bots, refetch: refetchBots } = useFindultralutionBot({
    instanceName: instance?.name,
    enabled: open,
  });
  const { setDefaultSettingsultralutionBot } = useManageultralutionBot();

  const form = useForm<z.infer<typeof FormSchema>>({
    resolver: zodResolver(FormSchema),
    defaultValues: {
      expire: "0",
      keywordFinish: t("ultralutionBot.form.examples.keywordFinish"),
      delayMessage: "1000",
      unknownMessage: t("ultralutionBot.form.examples.unknownMessage"),
  ...
  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    try {
      ...
      const settingsData: ultralutionBotSettings = {
      ...
      await setDefaultSettingsultralutionBot({
        instanceName: instance.name,
        token: instance.token,
        data: settingsData,
      });
      toast.success(t("ultralutionBot.toast.defaultSettings.success"));
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="secondary" size="sm">
          ...
          <span className="hidden sm:inline">{t("ultralutionBot.defaultSettings")}</span>
        </Button>
      </DialogTrigger>
      <DialogContent className="overflow-y-auto sm:max-h-[600px] sm:max-w-[740px]" onCloseAutoFocus={onReset}>
        <DialogHeader>
          <DialogTitle>{t("ultralutionBot.defaultSettings")}</DialogTitle>
        </DialogHeader>
        <FormProvider {...form}>
          <form className="w-full space-y-6" onSubmit={form.handleSubmit(onSubmit)}>
            <div>
              <div className="space-y-4">
                <FormSelect
                  name="botIdFallback"
                  label={t("ultralutionBot.form.botIdFallback.label")}
                  options={
                    bots
                      ?.filter((bot) => !!bot.id)
                      .map((bot) => ({
                ...
                <FormTags name="ignoreJids" label={t("ultralutionBot.form.ignoreJids.label")} placeholder={t("ultralutionBot.form.ignoreJids.placeholder")} />
              </div>
            </div>
            <DialogFooter>
              <Button type="submit">{t("ultralutionBot.button.save")}</Button>
            </DialogFooter>
          </form>
        </FormProvider>
      </DialogContent>
...
export { DefaultSettingsultralutionBot };
