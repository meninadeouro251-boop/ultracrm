...
import { FormSchemaType, ultraaiForm } from "./ultraaiForm";

function Newultraai({ resetTable }: { resetTable: () => void }) {
  const { t } = useTranslation();
  const { instance } = useInstance();

  const [updating, setUpdating] = useState(false);
  const [open, setOpen] = useState(false);
  const { createultraai } = useManageultraai();
  ...
  const onSubmit = async (data: FormSchemaType) => {
    try {
      ...
      toast.success(t("ultraai.toast.success.create"));
      setOpen(false);
      resetTable();
    } catch (error: any) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm">
          ...
          <span className="hidden sm:inline">{t("ultraai.button.create")}</span>
        </Button>
      </DialogTrigger>
      <DialogContent className="overflow-y-auto sm:max-h-[600px] sm:max-w-[740px]">
        <DialogHeader>
          <DialogTitle>{t("ultraai.form.title")}</DialogTitle>
        </DialogHeader>
        <ultraaiForm onSubmit={onSubmit} isModal={true} isLoading={updating} />
      </DialogContent>
    </Dialog>
  );
}
