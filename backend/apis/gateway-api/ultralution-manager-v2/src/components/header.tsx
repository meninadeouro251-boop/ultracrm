  ...
  return (
    <header className="flex items-center justify-between px-4 py-2">
      <Link to="/manager" onClick={navigateToDashboard} className="flex h-8 items-center gap-4">
        <img src={theme === "dark" ? "https://ultralution-api.com/files/ultra/ultralution-logo-white.svg" : "https://ultralution-api.com/files/ultra/ultralution-logo.svg"} alt="Logo" className="h-full" />
      </Link>
      <div className="flex items-center gap-4">
        {instanceId && (
          <Avatar className="h-8 w-8">
            <AvatarImage src={instance?.profilePicUrl || "/assets/images/ultralution-logo.png"} alt={instance?.name} />
          </Avatar>
        )}
        <LanguageToggle />
        <ModeToggle />
