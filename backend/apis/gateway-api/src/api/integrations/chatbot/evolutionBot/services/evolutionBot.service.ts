...
import { ultralutionBot, ultralutionBotSetting, IntegrationSession } from '@prisma/client';
...
export class ultralutionBotService extends BaseChatbotService<ultralutionBot, ultralutionBotSetting> {
  ...
  constructor(
    ...
    super(waMonitor, prismaRepository, 'ultralutionBotService', configService);
  ...
  /**
   * Send a message to the ultralution Bot API
   */
  protected async sendMessageToBot(
    instance: any,
    session: IntegrationSession,
    settings: ultralutionBotSetting,
    bot: ultralutionBot,
    remoteJid: string,
    pushName: string,
    content: string,
    ...
    try {
      ...
      if (this.isAudioMessage(content) && msg) {
        try {
          this.logger.debug(`[ultralutionBot] Downloading audio for Whisper transcription`);
          const transcription = await this.openaiService.speechToText(msg, instance);
          if (transcription) {
            payload.query = `[audio] ${transcription}`;
          }
        } catch (err) {
          this.logger.error(`[ultralutionBot] Failed to transcribe audio: ${err}`);
        }
      }
      ...
      if (!endpoint) {
        this.logger.error('No ultralution Bot endpoint defined');
        return;
      }
      ...
      if (message) {
        ...
        this.logger.warn(`[ultralutionBot] No message content received from bot response`);
      }
