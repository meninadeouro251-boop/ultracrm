...
import { ultraai, ultraaiSetting, IntegrationSession } from '@prisma/client';
...
export class ultraaiService extends BaseChatbotService<ultraai, ultraaiSetting> {
  ...
  constructor(
    ...
    super(waMonitor, prismaRepository, 'ultraaiService', configService);
    this.openaiService = openaiService;
  }

  /**
   * Return the bot type for ultraAI
   */
  ...
  protected async sendMessageToBot(
    ...
    try {
      this.logger.debug(`[ultraAI] Sending message to bot with content: ${content}`);
      ...
      if (this.isAudioMessage(content) && msg) {
        try {
          this.logger.debug(`[ultraAI] Downloading audio for Whisper transcription`);
          const transcription = await this.openaiService.speechToText(msg, instance);
          if (transcription) {
            processedContent = `[audio] ${transcription}`;
          }
        } catch (err) {
          this.logger.error(`[ultraAI] Failed to transcribe audio: ${err}`);
        }
      }

      const endpoint: string = ultraai.agentUrl;

      if (!endpoint) {
        this.logger.error('No ultraAI endpoint defined');
        return;
      }
      ...
      if (this.isImageMessage(content) && msg) {
        ...
        try {
          ...
          this.logger.error(`[ultraAI] Failed to process image: ${fileErr}`);
        }
      }
      ...
      this.logger.debug(`[ultraAI] Sending request to: ${endpoint}`);
      ...
      this.logger.debug(`[ultraAI] Payload: ${JSON.stringify(redactedPayload)}`);
      ...
      const response = await axios.post(endpoint, payload, {
        headers: {
          'x-api-key': ultraai.apiKey,
          'Content-Type': 'application/json',
        },
      });

      this.logger.debug(`[ultraAI] Response: ${JSON.stringify(response.data)}`);
      ...
      this.logger.debug(`[ultraAI] Extracted message to send: ${message}`);
      ...
      this.logger.error(
        `[ultraAI] Error sending message: ${error?.response?.data ? JSON.stringify(error.response.data) : error}`,
      );
      return;
    }
  }
