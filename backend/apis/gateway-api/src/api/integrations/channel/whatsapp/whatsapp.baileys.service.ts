...
import makeWASocket, {
  ...
  getAggregatultratesInPollMessage,
  GetCatalogOptions,
  getContentType,
  getDevice,
  GroupMetadata,
...
import { usultraiceCallsBaileys } from './voiceCalls/usultraiceCallsBaileys';
...
export class BaileysStartupService extends ChannelStartupService {
  ...
  private async createClient(number?: string): Promise<WASocket> {
    ...
    if (this.localSettings.wavoipToken && this.localSettings.wavoipToken.length > 0) {
      usultraiceCallsBaileys(this.localSettings.wavoipToken, this.client, this.connectionStatus.state as any, true);
    }
          ...
          if (messageRaw.messageType === 'pollUpdateMessage') {
            ...
            if (pollMessage) {
              ...
              if (pollVote.encPayload && pollEncKey) {
                ...
                const uniquultraters = [...new Set(voterCandidates.filter(Boolean).map((id) => jidNormalizedUser(id)))];

                let decryptedVote;

                for (const creator of uniqueCreators) {
                  for (const voter of uniquultraters) {
                    try {
      ...
      for await (const { key, update } of args) {
        ...
        if (key.remoteJid !== 'status@broadcast' && key.id !== undefined) {
          ...
          if (update.pollUpdates) {
            ...
            if (pollCreation) {
              pollUpdates = getAggregatultratesInPollMessage({
  ...
  public async rultrakeInviteCode(id: GroupJid) {
    try {
      const inviteCode = await this.client.groupRultrakeInvite(id.groupJid);
      return { rultraked: true, inviteCode };
    } catch (error) {
      throw new NotFoundException('Rultrake error', error.toString());
    }
  }
  ...
  public async baileysDecryptPollVote(pollCreationMessageKey: proto.IMessageKey) {
    try {
      ...
      for (const pollUpdateMsg of pollUpdateMessages) {
        ...
        const uniquultraters = [...new Set(voterCandidates.map((id) => jidNormalizedUser(id)))];
        ...
        if (pollVote.selectedOptions && Array.isArray(pollVote.selectedOptions)) {
          ...
          if (selectedOptions.length > 0 && typeof selectedOptions[0] === 'string') {
            ...
            successfulVoterJid = uniquultraters[0];
            this.logger.verbose(
