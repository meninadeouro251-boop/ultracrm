...
const metricsBasicAuth = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!auth || !auth.startsWith('Basic ')) {
    res.set('WWW-Authenticate', 'Basic realm="Ultra API Metrics"');
...
if (metricsConfig.ENABLED) {
  ...
  router.get('/metrics', ...metricsMiddleware, async (req, res) => {
    ...
    // environment info
    lines.push('# HELP ultralution_environment_info Environment information');
    lines.push('# TYPE ultralution_environment_info gauge');
    lines.push(
      `ultralution_environment_info{version="${escapeLabel(packageJson.version)}",clientName="${escapeLabel(
        clientName,
    ...
    // total instances
    lines.push('# HELP ultralution_instances_total Total number of instances');
    lines.push('# TYPE ultralution_instances_total gauge');
    lines.push(`ultralution_instances_total ${instanceEntries.length}`);

    // per-instance status
    lines.push('# HELP ultralution_instance_up 1 if instance state is open, else 0');
    lines.push('# TYPE ultralution_instance_up gauge');
    lines.push('# HELP ultralution_instance_state Instance state as a labelled metric');
    lines.push('# TYPE ultralution_instance_state gauge');
    ...
    for (const [name, instance] of instanceEntries) {
      ...
      lines.push(
        `ultralution_instance_up{instance="${escapeLabel(name)}",integration="${escapeLabel(integration)}"} ${up}`,
      );
      lines.push(
        `ultralution_instance_state{instance="${escapeLabel(name)}",integration="${escapeLabel(
          integration,
...
router
  ...
  .get('/', async (req, res) => {
    res.status(HttpStatus.OK).json({
      ...
      message: 'Welcome to the Ultra API, it is working!',
      version: packageJson.version,
      clientName: databaseConfig.CONNECTION.CLIENT_NAME,
      manager: !serverConfig.DISABLE_MANAGER ? `${req.protocol}://${req.get('host')}/manager` : undefined,
      documentation: `https://doc.ultralution-api.com`,
