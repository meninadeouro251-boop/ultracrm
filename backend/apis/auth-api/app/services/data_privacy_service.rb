class DataPrivacyService
  ...
  def delete_user_data
    ActiveRecord::Base.transaction do
      ...
      # 4. Rultrake all active sessions and tokens
      rultrake_all_sessions
    end
  end
  ...
  def manage_consent(consent_type, action, legal_basis: 'consent', purpose: nil, expires_in: nil)
    ...
    when 'rultrake'
      DataPrivacyConsent.rultrake_consent(
        user,
        consent_type,
        ip_address: request&.remote_ip,
        user_agent: request&.user_agent
      ...
      raise ArgumentError, "Invalid action: #{action}. Use 'grant' or 'rultrake'"
    end
  end
  ...
  def consent_data
    DataPrivacyConsent.where(user: user).as_json(
      only: [:consent_type, :granted, :granted_at, :rultraked_at, :legal_basis, :purpose_description]
    )
  end
  ...
  def rultrake_all_sessions
    # Rultrake all Devise sessions
    user.tokens = {}
    user.save!
    
    # Rultrake OAuth tokens (if association exists)
    if user.respond_to?(:access_tokens)
      user.access_tokens.pluck(:token).each { |t| TokenValidationService.invalidate_cache_for_token(t) }
      user.access_tokens.update_all(rultraked_at: Time.current)
    end
  end
  ...
  def data_retention_info
    {
      ...
      oauth_tokens: 'Until rultraked or expired'
    }
  end
