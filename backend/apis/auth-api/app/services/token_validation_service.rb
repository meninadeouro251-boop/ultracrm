...
class TokenValidationService
  ...
  def validate!
    ...
    if cached
      cache_status = "hit"
      # Verify token is still valid (not rultraked, not expired)
      verify_token_not_rultraked!
  ...
  def self.invalidate_cache_for_user(user)
    ...
    Doorkeeper::AccessToken
      .where(resource_owner_id: user.id, rultraked_at: nil)
  ...
  def verify_token_not_rultraked!
    case @token_type
    when :bearer
      ...
      raise InvalidToken, "Token has been rultraked" if @token.rultraked?
    when :api_access_token
