    ...
    class << self
      ...
      def full(user, options = {})
        ...
        base_data.merge!(
          ...
          setup_survey_completed: true
        )

        # Add hmac_identifier if ultraLUTION_INBOX_HMAC_KEY is present
        if GlobalConfigService.load('ultraLUTION_INBOX_HMAC_KEY').present? && user.respond_to?(:hmac_identifier)
