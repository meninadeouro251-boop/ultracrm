class Api::V1::DataPrivacyController < Api::BaseController
  ...
  def rultrake_consent
    consent_type = params[:consent_type]
    success = @privacy_service.manage_consent(consent_type, 'rultrake')

    if success
      success_response(data: {}, message: 'Consent rultraked successfully')
    else
      error_response('NOT_FOUND', 'Consent not found or already rultraked', status: :not_found)
    end
  end
  ...
  def user_rights_summary
    [
      ...
      { right: 'Object', description: 'Object to certain data processing', action: 'rultrake_consent', available: true }
    ]
  end
  ...
  def check_authorization
    action_map = {
      ...
      'rultrake_consent' => 'data_privacy.manage_consents',
