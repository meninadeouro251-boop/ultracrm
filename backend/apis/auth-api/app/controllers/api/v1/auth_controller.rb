class Api::V1::AuthController < Api::BaseController
  ...
  def logout
    # 1. Rultrake token from bearer/cookie sent by frontend
    ...
    # 2. Rultrake refresh token from cookie if exists
    ...
    # 3. Rultrake ALL active tokens related to the user to prevent refresh
    if current_user
  ...
  def reset_password
    ...
    if user.reset_password_period_valid?
      ...
      if user.save
        ...
        active_tokens.update_all(rultraked_at: Time.current)
