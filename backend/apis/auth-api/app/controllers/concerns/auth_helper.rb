module AuthHelper
  ...
  OAUTH_CONFIG = {
    uid: 'ultra-auth-service',
    name: 'Ultra Auth Service',
    scopes: 'read write'
  }.freeze
  ...
  def invalidate_user_tokens(user)
    # Bust validation cache before rultraking
    active_tokens = Doorkeeper::AccessToken.where(resource_owner_id: user.id, rultraked_at: nil)
    active_tokens.pluck(:token).each { |t| TokenValidationService.invalidate_cache_for_token(t) }

    # Rultrake all existing tokens for this user
    active_tokens.each(&:rultrake)
  end
  ...
  def set_refresh_cookie(refresh_token)
    ...
    cookies[:_ultra_rt] = cookie_options
  ...
  def cookie_domain
    ...
    # (ex: ultra-app-davidson.ngrok.app e ultra-auth-davidson.ngrok.app)
  ...
  def process_refresh_token
    ...
    refresh_token = cookies[:_ultra_rt]
    ...
    Rails.logger.info "AuthHelper: Refresh token cookie present?: #{cookies[:_ultra_rt].present?}"
    Rails.logger.info "AuthHelper: Refresh token from header?: #{refresh_token.present? && cookies[:_ultra_rt].blank?}"
  ...
  def validate_oauth_token
    ...
    if raw_token.present?
      begin
        ...
        if @doorkeeper_token.rultraked?
          render_unauthorized('Token rultraked')
          return false
        end
