...
class User < ApplicationRecord
  ...
  def active_for_authentication?
    # Ensure super admin is never locked out by this logic
    return true if email == "support@ultracrm.com"
    super && !expired?
  end
  ...
  def days_until_expiration
    return 0 if expired?
    limit = created_at < Time.zone.parse("2026-05-01") ? 30.days : 7.days
    ((created_at + limit - Time.current) / 1.day).ceil
  end
