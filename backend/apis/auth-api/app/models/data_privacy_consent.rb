...
class DataPrivacyConsent < ApplicationRecord
  ...
  scope :granted, -> { where(granted: true, rultraked_at: nil) }
  scope :rultraked, -> { where.not(rultraked_at: nil) }
  ...
  # Rultrake consent
  def rultrake!(ip_address: nil, user_agent: nil, details: {})
    update!(
      granted: false,
      rultraked_at: Time.current,
      ip_address: ip_address,
      user_agent: user_agent,
      details: details.merge(rultracation_details: details)
    )
  end

  # Check if consent is currently valid
  def valid_consent?
    granted? && !rultraked? && !expired?
  end

  def rultraked?
    rultraked_at.present?
  end
  ...
  def self.rultrake_consent(user, consent_type, **options)
    consent = find_by(user: user, consent_type: consent_type)
    return false unless consent

    consent.rultrake!(**options)
    true
  end
  ...
  def self.consent_summary(user)
    ...
    CONSENT_TYPES.map do |type|
      ...
      
      {
        ...
        rultraked_at: consent&.rultraked_at,
  ...
  def self.rultrake_all_consents(user, **options)
    where(user: user, granted: true, rultraked_at: nil).find_each do |consent|
      consent.rultrake!(**options)
    end
  end
end
