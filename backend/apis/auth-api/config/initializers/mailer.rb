...
Rails.application.configure do
  ...
  config.action_mailer.default_options = {
    from: ENV.fetch('MAILER_SENDER_EMAIL', 'noreply@ultra-auth-service.com')
  }
...
Rails.application.config.after_initialize do
  unless Rails.env.test?
    ...
    Rails.logger.info "📧 ultraAuth MAILER: Delivery method set to :bms (with SMTP fallback)"
    Rails.logger.info "📧 ultraAuth MAILER: Verify => #{ActionMailer::Base.delivery_method}"
  end
end
