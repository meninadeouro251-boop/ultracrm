...
class AgentFolder(Base):
    __tablename__ = "ultra_core_agent_folders"
...
class FolderShare(Base):
    __tablename__ = "ultra_core_folder_shares"
    __table_args__ = {"extend_existing": True, "info": {"skip_autogenerate": True}}

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    folder_id = Column(
        UUID(as_uuid=True),
        ForeignKey("ultra_core_agent_folders.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class Agent(Base):
    __tablename__ = "ultra_core_agents"
    __table_args__ = {"extend_existing": True, "info": {"skip_autogenerate": True}}
    ...
    api_key_id = Column(
        UUID(as_uuid=True),
        ForeignKey("ultra_core_api_keys.id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    folder_id = Column(
        UUID(as_uuid=True),
        ForeignKey("ultra_core_agent_folders.id", ondelete="SET NULL"),
        nullable=True,
    )
...
class MCPServer(Base):
    __tablename__ = "ultra_core_mcp_servers"
    __table_args__ = {"extend_existing": True, "info": {"skip_autogenerate": True}}
...
class Session(Base):
    __tablename__ = "ultra_ai_agent_processor_sessions"
...
class SessionMetadata(Base):
    __tablename__ = "ultra_ai_agent_processor_session_metadata"
...
class CustomMCPServer(Base):
    __tablename__ = "ultra_core_custom_mcp_servers"
    __table_args__ = {"info": {"skip_autogenerate": True}}
...
class ApiKey(Base):
    __tablename__ = "ultra_core_api_keys"
    __table_args__ = {"info": {"skip_autogenerate": True}}
...
class CustomTool(Base):
    __tablename__ = "ultra_core_custom_tools"
    __table_args__ = {"info": {"skip_autogenerate": True}}
...
class ExecutionMetrics(Base):
    __tablename__ = "ultra_ai_agent_processor_execution_metrics"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    agent_id = Column(UUID(as_uuid=True), ForeignKey("ultra_core_agents.id", ondelete="CASCADE"))
