...
class HttpMemoryService(BaseMemoryService):
    ...
    def _get_headers(self) -> Dict[str, str]:
        ...
        if settings.ultraAI_CRM_API_TOKEN:
            headers["X-Service-Token"] = settings.ultraAI_CRM_API_TOKEN
            logger.debug(f"Added X-Service-Token header for memory request")
        else:
            logger.warning("ultraAI_CRM_API_TOKEN not configured - service-to-service calls may fail")
