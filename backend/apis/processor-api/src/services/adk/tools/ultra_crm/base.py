"""
Base HTTP client for Ultra CRM API interactions.

This module provides a base client class for making authenticated requests
to the Ultra CRM API using service token authentication.
"""
...
class UltraCrmClient:
    """HTTP client for Ultra CRM API with service token authentication."""

    def __init__(self):
        """Initialize the Ultra CRM client with configuration from environment."""
        # Accept both ULTRA_CRM_API_TOKEN (new) and ultraAI_CRM_API_TOKEN (legacy)
        self.api_token = os.getenv("ULTRA_CRM_API_TOKEN") or os.getenv("ultraAI_CRM_API_TOKEN")
        # Accept both ULTRA_AI_CRM_URL (new) and ultra_AI_CRM_URL (legacy)
        self.base_url = os.getenv("ULTRA_AI_CRM_URL") or os.getenv("ultra_AI_CRM_URL", "http://localhost:3000")
        
        if not self.api_token:
            logger.warning(
                "ULTRA_CRM_API_TOKEN not set. CRM tools may not work correctly."
            )
        ...
        logger.info(f"UltraCrmClient initialized with base_url: {self.base_url}")
    ...
    async def _make_request(
        ...
        if not self.api_token:
            raise ValueError(
                "ULTRA_CRM_API_TOKEN not configured. Cannot make CRM API requests."
            )
