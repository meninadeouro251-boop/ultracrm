...
from src.services.adk.tools.ultra_crm.base import UltraCrmClient
...
def _extract_conversation_id_from_metadata(tool_context: ToolContext) -> Optional[str]:
    ...
    ultra_crm_data = tool_context.state.get("ultra_crm_data", {}) or tool_context.state.get("ultraai_crm_data", {})
    return ultra_crm_data.get("conversation_id") or ultra_crm_data.get("conversation", {}).get("id")
...
def _extract_contact_id_from_metadata(tool_context: ToolContext) -> Optional[str]:
    ...
    # Try ultra_crm_data (new) or ultraai_crm_data (legacy)
    ultra_crm_data = state.get("ultra_crm_data", {}) or state.get("ultraai_crm_data", {})
    if isinstance(ultra_crm_data, dict):
        # Try contact inside ultra_crm_data
        contact_data = ultra_crm_data.get("contact", {})
        if isinstance(contact_data, dict) and contact_data.get("id"):
            return str(contact_data.get("id"))
        
        # Try direct keys
        contact_id = ultra_crm_data.get("contactId") or ultra_crm_data.get("contact_id")
        if contact_id:
        ...
        try:
            ...
            try:
                ...
                if "404" in error_message or "not found" in error_message.lower():
                    ...
                    error_message = (
                        "Authentication failed. Please check ULTRA_CRM_API_TOKEN configuration."
                    )
