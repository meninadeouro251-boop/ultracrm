...
def _extract_conversation_id_from_metadata(tool_context: Optional[ToolContext]) -> Optional[str]:
    ...
    if isinstance(ultra_crm_data, dict):
        conversation_id = ultra_crm_data.get("conversation_id")
        if conversation_id:
            return str(conversation_id)

        conversation = ultra_crm_data.get("conversation", {})
...
async def _move_to_stage(
    ...
    pipeline_rule = next((r for r in pipeline_rules if r.get("pipelineId") == pipeline_id), None)
    resolved_stage_name = "unknown stage"
    if pipeline_rule:
        stage = next((s for s in pipeline_rule.get("stages", []) if s.get("stageId") == stage_id), None)
        if stage:
            resolved_stage_name = stage.get("stageName", resolved_stage_name)
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Successfully moved to stage '{resolved_stage_name}'",
            "action": "move_to_stage",
            "pipeline_id": pipeline_id,
            "stage_id": stage_id,
