...
│ Contact: contato@ultralution-api.com                                           │
├──────────────────────────────────────────────────────────────────────────────┤
│ @copyright © ultralution API 2025. All rights reserved.                        │
...
class GeminiWithApiKey(Gemini):
    ...
    def _live_api_client(self):
        ...
        #     "project_id": "ultralution-api-433821",
        ...
        #     "client_email": "ultra-ai@ultralution-api-433821.iam.gserviceaccount.com",
        #     "client_id": "104060719980706869243",
        #     "auth_uri": "https://accounts.google.com/o/oauth2/auth",
        #     "token_uri": "https://oauth2.googleapis.com/token",
        #     "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
        #     "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/ultra-ai%40ultralution-api-433821.iam.gserviceaccount.com",
        ...
        return Client(
            ...
            # credentials=creds,
            # vertexai=True,
            # project="ultralution-api-433821",
...
def create_update_contact_info_callback():
    ...
    async def update_contact_info(callback_context: CallbackContext):
        ...
        # The metadata is stored in state with keys like "ultraai_crm_data", "contact", etc.
        ...
        # 2. Check if contact is inside ultraai_crm_data
        if not contact_data:
            try:
                ultraai_crm_data = callback_context.state.get("ultraai_crm_data", {})
                logger.debug(f"[ContactInfo] ultraai_crm_data type: {type(ultraai_crm_data)}, keys: {list(ultraai_crm_data.keys()) if isinstance(ultraai_crm_data, dict) else 'N/A'}")
                if isinstance(ultraai_crm_data, dict) and "contact" in ultraai_crm_data:
                    contact_data = ultraai_crm_data.get("contact")
                    logger.info(f"[ContactInfo] ✅ Found contact inside ultraai_crm_data: {contact_data.get('name', 'Unknown') if isinstance(contact_data, dict) else 'N/A'}")
            except (AttributeError, TypeError) as e:
                logger.debug(f"[ContactInfo] Error accessing ultraai_crm_data: {e}")
        
        # 3. Check if ultraai_crm_data itself contains contact fields (flat structure)
        if not contact_data:
            try:
                ultraai_crm_data = callback_context.state.get("ultraai_crm_data", {})
                if isinstance(ultraai_crm_data, dict) and ultraai_crm_data.get("contactId"):
                    # Try to reconstruct contact from ultraai_crm_data fields
                    if ultraai_crm_data.get("contact"):
                        contact_data = ultraai_crm_data.get("contact")
                        logger.info(f"[ContactInfo] ✅ Found contact in ultraai_crm_data.contact: {contact_data.get('name', 'Unknown') if isinstance(contact_data, dict) else 'N/A'}")
            except (AttributeError, TypeError) as e:
                logger.debug(f"[ContactInfo] Error accessing ultraai_crm_data.contact: {e}")
