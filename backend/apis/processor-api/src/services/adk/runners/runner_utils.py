    ...
    async def setup_session_state(
        ...
        if metadata:
            ...
            for key, value in metadata.items():
                ...
                elif key == "ultraai_crm_data" and isinstance(value, dict) and "contact" in value:
                    contact_in_data = value.get("contact", {})
                    logger.info(f"[RunnerUtils] Storing ultraai_crm_data with contact: name={contact_in_data.get('name', 'N/A') if isinstance(contact_in_data, dict) else 'N/A'}")
