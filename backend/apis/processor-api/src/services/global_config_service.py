"""
Service to fetch global configuration from ultra-ai-crm.
...
class GlobalConfigService:
    """Service for fetching global configuration from ultra-ai-crm."""

    def __init__(self):
        """Initialize global config service."""
        self.crm_base_url = os.getenv("ultraLUTION_BASE_URL", "http://localhost:3000")
        self.api_token = os.getenv("ultraAI_CRM_API_TOKEN")
        self._config_cache: Optional[Dict[str, Any]] = None

        if not self.api_token:
            logger.warning("ultraAI_CRM_API_TOKEN not configured - global config fetching will fail")
