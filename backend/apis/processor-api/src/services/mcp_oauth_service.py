...
class MCPOAuthService:
    ...
    async def register_client(
        self,
        registration_endpoint: str,
        client_name: str = "ultraAI MCP Client",
    ...
    async def complete_authorization(
        ...
        try:
            ...
            # Store credentials via ultra-ai-core API (only if access_token was obtained)
            if access_token:
                try:
                    logger.info(
    ...
    async def _load_credentials(
        ...
        """Load OAuth credentials from database (preferred) or ultra-ai-core API."""
        # Prefer direct database access if db session is available
        if db is not None:
            try:
