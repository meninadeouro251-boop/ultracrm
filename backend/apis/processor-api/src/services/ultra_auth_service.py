...
│ @file: ultra_auth_service.py                                                 │
│ Developed by: Davidson Gomes                                                 │
│ Creation date: January 27, 2025                                              │
│ Contact: contato@ultracrm.com                                                │
├──────────────────────────────────────────────────────────────────────────────┤
│ @copyright © Ultra CRM 2025. All rights reserved.                            │
...
from src.schemas.auth import UltraAuthResponse
...
class UltraAuthService:
    """Service for Ultra Auth authentication validation and user management
    ...
    def __init__(self, ultra_auth_base_url: str = None):
        self.base_url = (ultra_auth_base_url or settings.ULTRA_AUTH_BASE_URL).rstrip("/")
    ...
    async def validate_token(self, token: str, token_type: str) -> Optional[UltraAuthResponse]:
        """Validate token with Ultra Auth API - Primary authentication method
        ...
        Returns:
            UltraAuthResponse with user and accounts data, or None if validation fails
        """
        headers = self.build_headers(token, token_type)
        
        logger.info(f"UltraAuth: Validating {token_type} token at {self.base_url}/api/v1/auth/validate")
        
        try:
            response_json = await self._post_request('/api/v1/auth/validate', {}, headers)
            
            if not response_json:
                logger.warning("UltraAuth: Empty response from validation endpoint")
                return None
            
            # Check for error in response
            if not response_json.get('success', False):
                error_info = response_json.get('error', {})
                logger.warning(f"UltraAuth: Validation failed: {error_info.get('message', 'Unknown error')}")
                return None
            
            # Extract data from response
            response_data = response_json.get('data')
            if not response_data:
                logger.warning(f"UltraAuth: Invalid response format - missing 'data' field")
                return None
            
            try:
                auth_response = UltraAuthResponse(**response_data)
                logger.info(f"Token validation successful for user: {auth_response.user.email}")
                return auth_response
            except (ValidationError, ValueError, KeyError) as e:
                logger.error(f"Validation error parsing UltraAuth response: {e}")
    ...
    async def _post_request(self, endpoint: str, payload: Dict[str, Any], headers: Dict[str, str] = None) -> Optional[Dict[str, Any]]:
        """Execute POST request to ultra-auth-service
        ...
        try:
            ...
            logger.error(f"UltraAuth: Request timeout for POST {endpoint}: {e}")
            raise NetworkError("Request timeout")
        except httpx.HTTPStatusError as e:
            logger.error(f"UltraAuth: HTTP error for POST {endpoint}: {e.response.status_code} - {e.response.text}")
            if e.response.status_code == 401:
                raise AuthenticationError("Invalid or expired token")
            raise NetworkError(f"HTTP error: {e.response.status_code}")
        except httpx.RequestError as e:
            logger.error(f"UltraAuth: Network error for POST {endpoint}: {e}")
            raise NetworkError(f"Network error: {e}")
        except Exception as e:
            logger.error(f"UltraAuth: Unexpected error for POST {endpoint}: {e}")
            return None


# Global instance
ultra_auth_service: Optional[UltraAuthService] = None

def get_auth_service() -> UltraAuthService:
    """Get or create singleton auth service instance"""
    global ultra_auth_service
    if ultra_auth_service is None:
        ultra_auth_service = UltraAuthService(ultra_auth_base_url=settings.ULTRA_AUTH_BASE_URL)
    return ultra_auth_service
