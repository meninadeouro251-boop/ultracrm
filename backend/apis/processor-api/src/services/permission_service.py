...
from src.services.ultra_auth_service import UltraAuthService
...
class PermissionService:
    ...
    Delegates permission checks to ultraAuthService for consistent behavior
    across all authentication and authorization operations.
    """
    
    def __init__(self, ultra_auth_base_url: str):
        self.ultra_auth_service = UltraAuthService(ultra_auth_base_url)
        logger.info(f"Permission service initialized with UltraAuthService")
    
    async def check_permission(self, auth_token: str, permission_key: str, token_type: str = "bearer") -> bool:
        """Check if user has specific permission via ultra-auth-service
        
        Delegates to UltraAuthService.check_permission for unified permission handling.
        ...
        """
        return await self.ultra_auth_service.check_permission(auth_token, permission_key, token_type)
    
    async def validate_permission(self, request: Request, resource: str, action: str) -> None:        
        # Build permission key
        permission_key = f"{resource}.{action}"
        
        # Get user context from request state (set by ultraAuthMiddleware)
        ...
        if not user_context:
            ...
            raise HTTPException(
                ...
                detail={
                    ...
                    "message": "User context not available. Ensure ultraAuthMiddleware is configured."
                }
            )
        ...
        # Validate permission via ultra-auth-service
...
def initialize_permission_service(ultra_auth_base_url: str) -> None:
    """Initialize the global permission service"""
    global _permission_service
    _permission_service = PermissionService(ultra_auth_base_url)
