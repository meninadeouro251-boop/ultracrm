...
async def get_jwt_token_ws(token: str, skip_validation: bool = False) -> Optional[dict]:
    """
    Verifies token for WebSocket using ultraAuth service.
    Returns the payload if the token is valid, None otherwise.
    """
    if skip_validation:
        # For WebSocket, we need to validate via UltraAuth service
        try:
            from src.services.ultra_auth_service import get_auth_service
            ...
            logger.warning(f"UltraAuth token validation failed: {str(e)}")
            return None
...
async def websocket_chat(
    ...
    try:
        ...
        # Try with token (Bearer token from UltraAuth)
