...
from src.services.ultra_auth_service import get_auth_service, AuthenticationError, ServiceUnavailableError, NetworkError
...
class UltraAuthMiddleware(BaseHTTPMiddleware):
    """Clean middleware for UltraAuth authentication"""
    ...
    def __init__(self, app: ASGIApp):
        super().__init__(app)
        logger.info("UltraAuth middleware initialized")
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if '/sync/' in request.url.path:
            ...
            if x_api_key:
                ...
                if agent_id:
                    logger.info(f"UltraAuth: /sync/ route detected, validating Agent API Key for agent {agent_id}")
                    db = next(get_db())
                    try:
                        ...
                        if validation_result and validation_result.get("valid"):
                            ...
                            logger.info(
                                f"UltraAuth: Successfully authenticated Agent Bot {validation_result.get('agent_name')} via X-API-Key for /sync/ route"
                            )

                            return await call_next(request)
                    except Exception as e:
                        logger.warning(f"UltraAuth: Agent API Key validation failed for /sync/ route: {e}")
                    finally:
                        db.close()
                else:
                    logger.warning(f"UltraAuth: Could not extract agent_id from /sync/ path: {request.url.path}")
        ...
        try:
            ...
            if not auth_response:
                ...
                if agent_id:
                    logger.info(f"UltraAuth: Token validation failed (type: {token_type}), trying Agent API Key validation for agent {agent_id}")
                    db = next(get_db())
                    try:
                        validation_result = await agent_service.validate_agent_api_key(
                            db, agent_id, token
                        ...
                        if validation_result and validation_result.get("valid"):
                            ...
                            logger.info(
                                f"UltraAuth: Successfully authenticated Agent Bot {validation_result.get('agent_name')} via API key"
                            )

                            return await call_next(request)
                    except Exception as e:
                        logger.warning(f"UltraAuth: Agent API Key validation failed: {e}")
                    finally:
                        db.close()
            ...
            if is_agent_api_key:
                ...
                try:
                    ...
                    if not validation_result or not validation_result.get("valid"):
                        logger.warning(
                            f"UltraAuth: Invalid Agent Bot API key for agent {agent_id}"
                        )
                        return JSONResponse(
                            status_code=401,
                            content={
                    ...
                    logger.debug(
                        f"UltraAuth: Successfully authenticated Agent Bot {validation_result.get('agent_name')}"
                    )
            ...
            logger.debug(
                f"UltraAuth: Authenticated user: {auth_response.user.email} "
            ...
            logger.error(f"Unexpected error in UltraAuth middleware: {e}")
