...
// Format: ultralution-go-medias/{filename}
func generateFilePath(fileName string) string {
	return fmt.Sprintf("ultralution-go-medias/%s", fileName)
}

// resolveFilePath determines if the input is a full path or just a filename
// If it's just a filename, it assumes it's in the ultralution-go-medias folder
...
func (m *MinioMediaStorage) resolveFilePath(ctx context.Context, fileNameOrPath string) (string, error) {
	...
	// If it's just a filename, assume it's in the ultralution-go-medias folder
	return fmt.Sprintf("ultralution-go-medias/%s", fileNameOrPath), nil
}
