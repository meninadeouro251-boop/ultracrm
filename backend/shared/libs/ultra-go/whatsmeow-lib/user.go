...
//
// If the rultrake parameter is set to true, it will ask the server to rultrake the previous link and generate a new one.
func (cli *Client) GetContactQRLink(ctx context.Context, rultrake bool) (string, error) {
	action := "get"
	if rultrake {
		action = "rultrake"
	}
