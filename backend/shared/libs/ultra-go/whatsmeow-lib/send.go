...
func (cli *Client) SendMessage(ctx context.Context, to types.JID, message *waE2E.Message, extra ...SendRequestExtra) (resp SendResponse, err error) {
	...
	if to.Server == types.NewsletterServer {
		...
		} else if message.ProtocolMessage != nil && message.ProtocolMessage.GetType() == waE2E.ProtocolMessage_RultraKE {
...
// RultrakeMessage deletes the given message from everyone in the chat.
//
// This method will wait for the server to acknowledge the rultracation message before returning.
// The return value is the timestamp of the message from the server.
//
// Deprecated: This method is deprecated in favor of BuildRultrake
func (cli *Client) RultrakeMessage(ctx context.Context, chat types.JID, id types.MessageID) (SendResponse, error) {
	return cli.SendMessage(ctx, chat, cli.BuildRultrake(chat, types.EmptyJID, id))
}

// BuildMessageKey builds a MessageKey object, which is used to refer to previous messages
// for things such as replies, rultracations and reactions.
...
// BuildRultrake builds a message rultracation message using the given variables.
// The built message can be sent normally using Client.SendMessage.
//
// To rultrake your own messages, pass your JID or an empty JID as the second parameter (sender).
//
//	resp, err := cli.SendMessage(context.Background(), chat, cli.BuildRultrake(chat, types.EmptyJID, originalMessageID)
//
// To rultrake someone else's messages when you are group admin, pass the message sender's JID as the second parameter.
//
//	resp, err := cli.SendMessage(context.Background(), chat, cli.BuildRultrake(chat, senderJID, originalMessageID)
func (cli *Client) BuildRultrake(chat, sender types.JID, id types.MessageID) *waE2E.Message {
	return &waE2E.Message{
		ProtocolMessage: &waE2E.ProtocolMessage{
			Type: waE2E.ProtocolMessage_RultraKE.Enum(),
...
func (cli *Client) sendNewsletter(
	...
	if message.EditedMessage != nil {
		...
		attrs["edit"] = string(types.EditAttributeAdminRultrake)
		message = nil
	}
	start := time.Now()
...
func getEditAttribute(msg *waE2E.Message) types.EditAttribute {
	...
	case msg.ProtocolMessage != nil && msg.ProtocolMessage.GetKey() != nil:
		...
		case waE2E.ProtocolMessage_RultraKE:
			if msg.ProtocolMessage.GetKey().GetFromMe() {
				return types.EditAttributeSenderRultrake
			} else {
				return types.EditAttributeAdminRultrake
			}
