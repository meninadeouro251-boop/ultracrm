...
Guia de práticas de segurança para ambientes de produção do ultralution GO.
...
http {
    ...
    server {
        location / {
            ...
            proxy_pass http://ultralution-go:4000;
        }
    }
}
```
...
echo "$(uuidgen)" | docker secret create ultra_api_key -
...
services:
  ultralution-go:
    secrets:
      ...
      - postgres_password
    environment:
      GLOBAL_API_KEY_FILE: /run/secrets/ultra_api_key
...
RUN addgroup -g 1000 ultralution && \
    adduser -D -u 1000 -G ultralution ultralution

WORKDIR /app
COPY --chown=ultralution:ultralution server .
...
```bash
# Trivy
trivy image ultraapicloud/ultralution-go:latest

# Apenas críticas
trivy image --severity CRITICAL ultraapicloud/ultralution-go:latest

# Docker Scout
docker scout cves ultraapicloud/ultralution-go:latest
```
...
# Gerar certificado
sudo certbot certonly --standalone -d ultralution.seudominio.com

# Certificados em:
# /etc/letsencrypt/live/ultralution.seudominio.com/
```
...
server {
    listen 80;
    server_name ultralution.seudominio.com;
...
server {
    ...
    # Certificados
    ssl_certificate /etc/letsencrypt/live/ultralution.seudominio.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/ultralution.seudominio.com/privkey.pem;
    ...
    ssl_trusted_certificate /etc/letsencrypt/live/ultralution.seudominio.com/chain.pem;
...
# Apenas SSL
hostssl ultrago_auth ultralution 10.0.0.0/8 scram-sha-256
hostssl ultrago_users ultralution 10.0.0.0/8 scram-sha-256
...
docker run --rm \
  ...
  gpg --encrypt --recipient $GPG_KEY --output "${BACKUP_DIR}/ultralution_data_${TIMESTAMP}.tar.gz.gpg"
...
ultralution GO usa GORM (ORM) que previne SQL injection por padrão através de prepared statements.
...
- Rultragar credenciais comprometidas
