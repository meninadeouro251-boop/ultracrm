...
Métodos de instalação do ultralution GO para diferentes ambientes.
...
```bash
git clone https://git.ultraai.app/ultralution/ultralution-go.git
cd ultralution-go
```
...
```bash
mkdir ultralution-go-deploy && cd ultralution-go-deploy

curl -o docker-compose.yml https://raw.githubusercontent.com/ultralutionAPI/ultralution-go/main/docker/examples/docker-compose.yml
curl -o init-db.sql https://raw.githubusercontent.com/ultralutionAPI/ultralution-go/main/docker/examples/init-db.sql
```
...
POSTGRES_AUTH_DB=postgresql://postgres:postgres@localhost:5432/ultrago_auth?sslmode=disable
POSTGRES_USERS_DB=postgresql://postgres:postgres@localhost:5432/ultrago_users?sslmode=disable
...
Ou:
```bash
go run cmd/ultralution-go/main.go -dev
```

**Build produção:**
```bash
make build-local
./build/ultralution-go
```
...
```bash
curl -o docker-compose-full.yml https://raw.githubusercontent.com/ultralutionAPI/ultralution-go/main/docker/examples/docker-compose.full.yml
...
|---------|-------|--------|
| ultralution GO | 4000 | API principal |
...
3. Create bucket: `ultralution-media`
...
**Local:**
```bash
tail -f logs/ultralution-go.log
```
...
2. Login: minioadmin / minioadmin
3. Criar bucket: `ultralution-media`

Ou via CLI:
```bash
docker-compose exec minio mc mb /data/ultralution-media
```
...
# Escalar
docker service scale ultralution_ultralution_go=3

# Atualizar
docker service update --image ultraapicloud/ultralution-go:latest ultralution_ultralution_go
