...
Guia completo para desenvolver e contribuir com o ultralution GO.
...
O ultralution GO é um **gateway de API WhatsApp** escrito em Go, utilizando:
...
git clone git@git.ultrachat.com:ultralution/ultralution-go.git
cd ultralution-go
```
...
[GIN-debug] POST   /instance/create          --> ultralution-go/pkg/instance.(*InstanceHandler).Create-fm (4 handlers)
...
...
```json
{
  ...
  "configurations": [
    {
      "name": "Launch ultralution GO",
      "type": "go",
      "request": "launch",
      "mode": "debug",
      "program": "${workspaceFolder}/cmd/ultralution-go",
      "args": ["-dev"],
      "env": {
        "WADEBUG": "DEBUG"
      },
...
3. Configurar:
   - **Name**: ultralution GO Dev
   - **Run kind**: Directory
   - **Directory**: `cmd/ultralution-go`
...
```go
import "ultralution-go/pkg/utils/logger"
...
**Mantido por**: Equipe ultraAI Services  
**Versão**: 1.0.0
