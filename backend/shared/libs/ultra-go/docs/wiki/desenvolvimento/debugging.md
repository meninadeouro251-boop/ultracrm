...
Guia para resolver problemas comuns no ultralution GO.
...
docker-compose logs ultralution-go | grep ERROR
```
...
# Testar conexão
psql -h localhost -U postgres -d ultrago_auth
```

### 3. Erro: "database does not exist"

**Causa**: Databases `ultrago_auth` ou `ultrago_users` não criados.
...
```bash
# Verificar logs
docker-compose logs -f ultralution-go | grep QR
...
# Verificar logs
docker-compose logs ultralution-go | grep webhook
...
2. Selecionar "Launch ultralution GO"
...
ALTER DATABASE ultrago_users SET log_min_duration_statement = 1000;
...
WHERE datname IN ('ultrago_auth', 'ultrago_users');
```
...
# Debug
dlv debug cmd/ultralution-go/main.go -- -dev
