...
Sistema de armazenamento de arquivos de mídia do ultralution GO. Suporta MinIO, Amazon S3 e outros serviços compatíveis com S3.
...
O ultralution GO armazena arquivos de mídia (imagens, vídeos, áudios, documentos) em **object storage** compatível com S3. Isso inclui serviços como MinIO, Amazon S3, Backblaze B2, DigitalOcean Spaces e outros.
...
# Bucket
MINIO_BUCKET=ultralution-go-media
...
docker run --rm   --network host   minio/mc mb local/ultralution-go-media
...
MINIO_BUCKET=meu-bucket-ultralution
MINIO_REGION=us-east-1
MINIO_USE_SSL=true
```
...
Todos os arquivos são armazenados automaticamente no diretório `ultralution-go-medias/`:

**Exemplos**:
- Arquivo: `photo-123.jpg` → Caminho: `ultralution-go-medias/photo-123.jpg`
- Arquivo: `video-456.mp4` → Caminho: `ultralution-go-medias/video-456.mp4`
- Arquivo: `document-789.pdf` → Caminho: `ultralution-go-medias/document-789.pdf`
...
Quando você armazena ou solicita acesso a um arquivo, o ultralution GO gera automaticamente uma URL presignada com validade de 7 dias.

**Exemplo de URL presignada**:
```
https://s3.amazonaws.com/ultralution-go-media/ultralution-go-medias/photo-123.jpg?
...
1. O ultralution GO baixa o arquivo
...
O ultralution GO configura automaticamente o Content-Type baseado na extensão do arquivo:
- `.jpg`, `.jpeg` → `image/jpeg`
- `.png` → `image/png`
- `.mp4` → `video/mp4`
- `.pdf` → `application/pdf`
...
**MinIO**:
```bash
mc ilm add local/ultralution-go-media   --prefix "ultralution-go-medias/"   --expiry-days 30
```
...
3. URLs ficam: `https://d123456.cloudfront.net/ultralution-go-medias/photo.jpg`
...
# MinIO
mc du local/ultralution-go-media
```
...
```bash
# Criar bucket
mc mb test/ultralution-go-media

# Ou via AWS CLI
aws s3 mb s3://ultralution-go-media
```
...
**Documentação gerada para ultralution GO v1.0**
