...
Como funciona o processo de autenticação via QR Code no ultralution GO.
...
O ultralution GO utiliza o protocolo Multi-Device do WhatsApp para autenticar instâncias. O processo é baseado em QR Code, similar ao WhatsApp Web.

### Como Funciona

1. **Gerar QR Code**: O ultralution GO cria um código único
2. **Escanear**: Você escaneia o código com o WhatsApp no celular
3. **Pareamento**: WhatsApp e ultralution GO trocam chaves de criptografia
...
O ultralution GO solicita ao WhatsApp um código QR único, que contém:
...
1. **ultralution GO recebe confirmação** do WhatsApp
...
O ultralution GO gera um código de 8 dígitos único e o retorna na API. Esse código é válido por tempo limitado e pode ser usado uma única vez.
...
O ultralution GO mantém várias tabelas para armazenar as informações de sessão e chaves criptográficas necessárias para reconexão.

### Tentativas de Reconexão

Se uma conexão cair, o ultralution GO tenta reconectar automaticamente usando estratégia de intervalo crescente:
