...
Entenda como funciona o protocolo Multi-Device do WhatsApp e como o ultralution GO o utiliza.
...
└──────┬──────┘
       ...
       ▼            ▼            ▼            ▼
┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐
│WhatsApp  │ │ultralution │ │WhatsApp  │ │WhatsApp  │
...
2. **Dispositivos Companion (Companion Devices)**
   - WhatsApp Web, Desktop, Business API, ultralution GO
...
└───────────────────┬──────────────────────────────────────┘
    ...
    │(Phone) │ │Desktop │ │ultralution│ │   Web  │ │Business│
    └────────┘ └────────┘ └────────┘ └────────┘ └────────┘
         │          │          │          │          │
         └──────────┴──────────┴──────────┴──────────┘
                    │
...
**Enviar Mensagem**:
1. Device 1 (ultralution GO) envia mensagem criptografada para WhatsApp servers
...
**Limitação atual**: ultralution GO **não suporta atender** chamadas de voz/vídeo.
...
**ultralution GO funciona mesmo com celular offline** (após sincronização inicial).
...
**Exemplo**:
- Enviar mensagem no ultralution GO
...
**Com containers Docker**, você pode ter **N instâncias** do ultralution GO, cada uma conectada a um número WhatsApp diferente:

```bash
docker run -d --name ultra-vendas ultralution-go
docker run -d --name ultra-suporte ultralution-go
docker run -d --name ultra-marketing ultralution-go
```
...
**Load Balancing**: Distribua carga entre múltiplos ultralution GO instances.
