...
Sistema completo de notificações em tempo real do ultralution GO. Receba eventos do WhatsApp através de múltiplos canais: Webhooks, RabbitMQ, NATS e WebSocket.
...
O ultralution GO envia notificações de eventos do WhatsApp em tempo real através de diferentes canais de comunicação:
...
Envia eventos via HTTP POST para uma URL que você configurar. É o método mais simples para integrar o ultralution GO com suas aplicações.
...
3. **Retry Automático**: Se a requisição falhar, o ultralution GO tenta novamente até 5 vezes
...
As filas RabbitMQ criadas pelo ultralution GO são configuradas com:
...
```
ultralution.{instance}.{event_type}
```

**Exemplos**:
- `ultralution.vendas.message` - Mensagens da instância "vendas"
- `ultralution.suporte.calloffer` - Chamadas da instância "suporte"
- `ultralution.*.message` - Mensagens de todas as instâncias (wildcard)
...
func main() {
    ...
    sub, err := nc.Subscribe("ultralution.*.message", func(m *nats.Msg) {
...
O ultralution GO gerencia automaticamente as conexões WebSocket:
...
O ultralution GO usa dois níveis de classificação de eventos:
...
4. ✅ Tópico NATS `ultralution.vendas.message`
...
<html>
<head>
    <title>ultralution GO Dashboard</title>
</head>
<body>
