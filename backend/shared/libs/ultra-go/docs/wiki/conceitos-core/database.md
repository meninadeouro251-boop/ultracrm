...
Sistema de armazenamento de dados do ultralution GO usando bancos separados para diferentes finalidades.
...
O ultralution GO usa **dois bancos de dados separados**. Pense nisso como ter dois armários diferentes:

1. **Banco Auth** (`ultrago_auth`): Guarda os dados técnicos do WhatsApp
2. **Banco Users** (`ultrago_users`): Guarda os dados da sua aplicação
...
POSTGRES_AUTH_DB=postgresql://user:pass@localhost:5432/ultrago_auth

# Banco Users (Dados da API)
POSTGRES_USERS_DB=postgresql://user:pass@localhost:5432/ultrago_users
```

### Criação Automática de Tabelas

Quando você inicia o ultralution GO pela primeira vez:
...
pg_dump -U postgres ultrago_auth > backup_auth_$(date +%Y%m%d).sql

# Backup do banco Users
pg_dump -U postgres ultrago_users > backup_users_$(date +%Y%m%d).sql
```
...
2. Reduza o número de conexões máximas no ultralution GO
